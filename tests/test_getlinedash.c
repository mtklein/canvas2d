...
int main(void) {
    ...
    CHECK(canvas2d_get_line_dash(cv, NULL, 0) == 4);
    ...
    CHECK(canvas2d_get_line_dash(cv, two, 2) == 4);
    ...
    out[0] = 999.0f;
    CHECK(canvas2d_get_line_dash(cv, out, 8) == 4);
    CHECK(fabsf(out[0] - 4.0f) < 1e-6f);

    // Clearing the dash returns to empty.
    canvas2d_set_line_dash(cv, NULL, 0);
    CHECK(canvas2d_get_line_dash(cv, out, 8) == 0);
