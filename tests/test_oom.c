...
// -Dmalloc=canvas2d_oom_malloc (and realloc/calloc), so canvas2d_oom_fail_at(k) makes the
...
typedef void (*scene_fn)(struct canvas2d_context *__single cv);

static void scene_fill(struct canvas2d_context *__single cv) {
    canvas2d_begin_path(cv);
    canvas2d_move_to(cv, 4.0f, 4.0f);
    canvas2d_line_to(cv, 28.0f, 6.0f);
    canvas2d_line_to(cv, 16.0f, 28.0f);
    canvas2d_close_path(cv);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.8f, 0.3f, 0.2f, 1.0f);
    canvas2d_fill(cv, CANVAS2D_NONZERO);
}

static void scene_curve(struct canvas2d_context *__single cv) {
    canvas2d_begin_path(cv);
    canvas2d_move_to(cv, 2.0f, 16.0f);
    canvas2d_bezier_curve_to(cv, 10.0f, 0.0f, 22.0f, 32.0f, 30.0f, 16.0f);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.3f, 0.7f, 0.4f, 1.0f);
    canvas2d_fill(cv, CANVAS2D_NONZERO);
}

static void scene_gradient(struct canvas2d_context *__single cv) {
    canvas2d_set_fill_linear_gradient(cv, CANVAS2D_CS_SRGB, CANVAS2D_ALPHA_UNPREMUL, 0.0f, 0.0f, (float)W, (float)H);
...
static void scene_stroke(struct canvas2d_context *__single cv) {
    canvas2d_begin_path(cv);
    canvas2d_move_to(cv, 2.0f, 2.0f);
    canvas2d_line_to(cv, 30.0f, 8.0f);
    canvas2d_line_to(cv, 10.0f, 30.0f);
    canvas2d_set_line_width(cv, 3.0f);
    canvas2d_set_stroke_rgba(cv, CANVAS2D_CS_SRGB, 0.2f, 0.6f, 0.9f, 1.0f);
    canvas2d_stroke(cv);
}

static void scene_image(struct canvas2d_context *__single cv) {
    uint8_t buf[16 * 16 * 4];
    canvas2d_get_image_data(cv, CANVAS2D_CS_SRGB, 0, 0, 16, 16, buf, (int)sizeof buf);
    canvas2d_put_image_data(cv, CANVAS2D_CS_SRGB, buf, (int)sizeof buf, 16, 16, 4, 4);
    uint8_t src[8 * 8 * 4];
    for (int i = 0; i < (int)sizeof src; i++) {
        src[i] = (uint8_t)(i * 7);
    }
    canvas2d_draw_bitmap_scaled(cv, CANVAS2D_CS_SRGB, src, 8, 8, 2.0f, 2.0f, 24.0f, 24.0f);
}

// Clipping: build a clip mask, then paint through it (exercises the canvas's
// clip-mask allocation and the clip stack).
static void scene_clip(struct canvas2d_context *__single cv) {
    canvas2d_save(cv);
    canvas2d_begin_path(cv);
    canvas2d_arc(cv, 16.0f, 16.0f, 12.0f, 0.0f, 6.2831853f, false);
    canvas2d_clip(cv, CANVAS2D_NONZERO);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.9f, 0.6f, 0.2f, 1.0f);
    canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_restore(cv);
}
...
static void scene_text(struct canvas2d_context *__single cv) {
    canvas2d_set_font_size(cv, 14.0f);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.9f, 0.9f, 0.95f, 1.0f);
    canvas2d_fill_text(cv, "Ag", 2.0f, 20.0f);
    canvas2d_fill_text(cv, "Ag", 2.0f, 20.0f);     // warm: shape + glyph hits
    (void)canvas2d_measure_text(cv, "Ag");         // measure-then-draw shares the line
    canvas2d_fill_text(cv, "g A", 2.0f, 30.0f);    // new key; shared glyphs, a blank
    canvas2d_fill_text(cv, "\xF0\x9F\x99\x82", 2.0f, 30.0f);  // 🙂: capture + mips
    canvas2d_fill_text(cv, "\xF0\x9F\x99\x82", 12.0f, 30.0f); // warm capture hit
}

// Image pattern fill (createPattern path): a small tile sampled under the CTM.
static void scene_pattern(struct canvas2d_context *__single cv) {
    uint8_t tile[4 * 4 * 4];
    for (int i = 0; i < (int)sizeof tile; i++) {
        tile[i] = (uint8_t)(i * 11);
    }
    canvas2d_set_fill_pattern(cv, CANVAS2D_CS_SRGB, tile, 4, 4, CANVAS2D_REPEAT);
    canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
}

// Dashed stroke: the dash pattern is copied into the canvas state, then stroking
// walks it.
static void scene_dash(struct canvas2d_context *__single cv) {
    float const dash[4] = { 5.0f, 3.0f, 2.0f, 3.0f };
    canvas2d_set_line_dash(cv, dash, 4);
    canvas2d_set_line_width(cv, 2.0f);
    canvas2d_set_stroke_rgba(cv, CANVAS2D_CS_SRGB, 0.8f, 0.8f, 0.3f, 1.0f);
    canvas2d_begin_path(cv);
    canvas2d_move_to(cv, 2.0f, 16.0f);
    canvas2d_line_to(cv, 30.0f, 16.0f);
    canvas2d_stroke(cv);
}

// save/restore nesting grows the state stack.
static void scene_savestack(struct canvas2d_context *__single cv) {
    for (int i = 0; i < 12; i++) {
        canvas2d_save(cv);
        canvas2d_translate(cv, 1.0f, 1.0f);
        canvas2d_set_global_alpha(cv, 0.9f);
    }
    for (int i = 0; i < 12; i++) {
        canvas2d_restore(cv);
    }
...
static void scene_filter(struct canvas2d_context *__single cv) {
    canvas2d_add_filter_grayscale(cv, 1.0f);
    canvas2d_add_filter_brightness(cv, 1.2f);
    canvas2d_save(cv);
    canvas2d_add_filter_invert(cv, 0.5f);
    canvas2d_add_filter_blur(cv, 2.0f);
    canvas2d_add_filter_drop_shadow(cv, CANVAS2D_CS_SRGB, 2.0f, 2.0f, 1.0f, 0.1f, 0.2f, 0.3f, 0.8f);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.9f, 0.4f, 0.2f, 0.8f);
    canvas2d_fill_rect(cv, 4.0f, 4.0f, 24.0f, 24.0f);
    canvas2d_restore(cv);
    canvas2d_set_filter_none(cv);
}

// Recording (canvas2d_record_to): the write-side recorder's own allocation sites
// -- the recorder struct (calloc in canvas2d_recorder_begin), each image block's
// pixel copy and its deflate scratch (canvas2d_rec_image), a Path2D block's command
// copy (canvas2d_rec_path), and the text blocks' deflate buffer (canvas2d_rec_text_blocks)
...
static void scene_record(struct canvas2d_context *__single cv) {
    if (!canvas2d_record_to(cv, "build/test_oom_rec.canvas")) {
    ...
    canvas2d_draw_bitmap_scaled(cv, CANVAS2D_CS_SRGB, src, 8, 8, 2.0f, 2.0f, 16.0f, 16.0f);
    ...
    if (p) {
        canvas2d_path2d_move_to(p, 2.0f, 2.0f);
        canvas2d_path2d_line_to(p, 28.0f, 6.0f);
        canvas2d_path2d_quadratic_curve_to(p, 30.0f, 16.0f, 20.0f, 28.0f);
...
static void scene_pointinpath(struct canvas2d_context *__single cv) {
    canvas2d_begin_path(cv);
    canvas2d_arc(cv, 16.0f, 16.0f, 10.0f, 0.0f, 6.2831853f, false);
    (void)canvas2d_is_point_in_path(cv, 16.0f, 16.0f, CANVAS2D_NONZERO);
    (void)canvas2d_is_point_in_path(cv, 0.0f, 0.0f, CANVAS2D_NONZERO);
}

// PNG encode: exercises canvas2d_png's raw/zlib buffer allocations (two of the sites
// fixed alongside this harness).  Output is discarded.
static void scene_png(struct canvas2d_context *__single cv) {
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.3f, 0.5f, 0.7f, 1.0f);
    canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    (void)canvas2d_write_png(cv, "/dev/null");
}
...
static void sweep(scene_fn fn) {
    struct canvas2d_context *__single probe = canvas2d(W, H, CANVAS2D_CS_SRGB);
    CHECK(probe != NULL);
    if (!probe) {
        return;
    }
    canvas2d_oom_fail_at(0);
    fn(probe);
    int const allocs = canvas2d_oom_seen();
    canvas2d_free(probe);
    CHECK(allocs > 0);  // the scene must allocate, or the sweep tests nothing

    for (int k = 1; k <= allocs; k++) {
        struct canvas2d_context *__single cv = canvas2d(W, H, CANVAS2D_CS_SRGB);  // fresh -> stable alloc sequence
        if (!cv) {
            continue;
        }
        canvas2d_oom_fail_at(k);
        fn(cv);                                      // k-th allocation fails
        canvas2d_oom_fail_at(0);
        canvas2d_clear_rect(cv, 0.0f, 0.0f, (float)W, (float)H);  // still usable
        canvas2d_free(cv);
    }
}
...
int main(void) {
    ...
    for (int k = 1; k <= 12; k++) {
        ...
        canvas2d_oom_fail_at(0);
        if (cv) {
            canvas2d_free(cv);  // k past create's alloc count -> a real canvas
        }
    }
    ...
    if (cv) {
        ...
        canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
        uint8_t px[4];
        canvas2d_get_image_data(cv, CANVAS2D_CS_SRGB, W / 2, H / 2, 1, 1, px, 4);
