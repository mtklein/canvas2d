...
// may be translucent.
static struct rgba over(struct canvas2d_context *__single cv, uint8_t *__counted_by(LEN) px,
    ...
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, r2, g2, b2, a2);
    canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_set_global_composite_operation(cv, op);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, r1, g1, b1, a1);
    canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
...
static struct rgba blend(struct canvas2d_context *__single cv, uint8_t *__counted_by(LEN) px,
                        enum canvas2d_composite_op op, float br, float bg, float bb,
...
int main(void) {
    ...
    enum canvas2d_composite_op const SYM[] = {
        CANVAS2D_OP_MULTIPLY, CANVAS2D_OP_SCREEN, CANVAS2D_OP_DARKEN,
        CANVAS2D_OP_LIGHTEN, CANVAS2D_OP_DIFFERENCE, CANVAS2D_OP_EXCLUSION,
    };
    ...
    for (int i = 0; i < NCOL; i++) {
        for (int j = 0; j < NCOL; j++) {
            struct rgba ov = blend(cv, px, CANVAS2D_OP_OVERLAY, COL[i][0], COL[i][1], COL[i][2],
                                  COL[j][0], COL[j][1], COL[j][2], 1.0f);
            struct rgba hl = blend(cv, px, CANVAS2D_OP_HARD_LIGHT, COL[j][0], COL[j][1], COL[j][2],
    ...
    enum canvas2d_composite_op const HSL[] = {
    ...
    for (int m = 0; m < (int)(sizeof HSL / sizeof HSL[0]); m++) {
        for (int i = 0; i < NCOL; i++) {
            struct rgba self = blend(cv, px, HSL[m], COL[i][0], COL[i][1], COL[i][2],
                                    COL[i][0], COL[i][1], COL[i][2], 1.0f);
            struct rgba want = blend(cv, px, CANVAS2D_OP_COPY, 0.0f, 0.0f, 0.0f,
                                   COL[i][0], COL[i][1], COL[i][2], 1.0f);
    ...
    for (int i = 0; i < NCOL; i++) {
        ...
        struct rgba s = blend(cv, px, CANVAS2D_OP_COPY, br, bgc, bb, sr, sg, sb, 1.0f);  // == S
        struct rgba b = blend(cv, px, CANVAS2D_OP_COPY, sr, sg, sb, br, bgc, bb, 1.0f);  // == B
        enum canvas2d_composite_op const TO_S[] = { CANVAS2D_OP_SOURCE_IN, CANVAS2D_OP_SOURCE_ATOP,
                                             CANVAS2D_OP_SOURCE_OVER };
        enum canvas2d_composite_op const TO_B[] = { CANVAS2D_OP_DESTINATION_OVER, CANVAS2D_OP_DESTINATION_IN,
                                             CANVAS2D_OP_DESTINATION_ATOP };
        for (int k = 0; k < 3; k++) {
    ...
    for (int i = 0; i < NCOL; i++) {
        float sr = COL[i][0], sg = COL[i][1], sb = COL[i][2];
        struct rgba s = blend(cv, px, CANVAS2D_OP_COPY, 0, 0, 0, sr, sg, sb, 1.0f);
        CHECK(px_near(blend(cv, px, CANVAS2D_OP_MULTIPLY, 1, 1, 1, sr, sg, sb, 1.0f),
                      s.r, s.g, s.b, s.a, 1));
        CHECK(px_near(blend(cv, px, CANVAS2D_OP_SCREEN, 0, 0, 0, sr, sg, sb, 1.0f),
                      s.r, s.g, s.b, s.a, 1));
    }

    // 6. Transform translation invariance: a disc drawn at an integer offset matches
    //    the same disc under canvas2d_translate of that offset.  Drives the CTM path.
    canvas2d_set_global_composite_operation(cv, CANVAS2D_OP_SOURCE_OVER);
    canvas2d_reset_transform(cv);
    canvas2d_clear_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.9f, 0.4f, 0.2f, 1.0f);
    canvas2d_begin_path(cv);
    canvas2d_arc(cv, 9.0f, 9.0f, 4.0f, 0.0f, 6.2831853f, false);
    canvas2d_fill(cv, CANVAS2D_NONZERO);
    canvas2d_read_rgba(cv, CANVAS2D_CS_SRGB, px, LEN);          // disc at (9,9), direct
    canvas2d_clear_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_save(cv);
    canvas2d_translate(cv, 4.0f, 3.0f);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.9f, 0.4f, 0.2f, 1.0f);
    canvas2d_begin_path(cv);
    canvas2d_arc(cv, 5.0f, 6.0f, 4.0f, 0.0f, 6.2831853f, false);  // 5+4=9, 6+3=9
    canvas2d_fill(cv, CANVAS2D_NONZERO);
    canvas2d_restore(cv);
    canvas2d_read_rgba(cv, CANVAS2D_CS_SRGB, qx, LEN);          // disc at (9,9), via translate
    CHECK(memcmp(px, qx, (size_t)LEN) == 0);

    // 7. fill_rect is equivalent to filling the same rectangle as a path.
    canvas2d_reset_transform(cv);
    canvas2d_clear_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.3f, 0.7f, 0.9f, 1.0f);
    canvas2d_fill_rect(cv, 2.0f, 3.0f, 9.0f, 7.0f);
    canvas2d_read_rgba(cv, CANVAS2D_CS_SRGB, px, LEN);
    canvas2d_clear_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.3f, 0.7f, 0.9f, 1.0f);
    canvas2d_begin_path(cv);
    canvas2d_rect(cv, 2.0f, 3.0f, 9.0f, 7.0f);
    canvas2d_fill(cv, CANVAS2D_NONZERO);
    canvas2d_read_rgba(cv, CANVAS2D_CS_SRGB, qx, LEN);
    CHECK(memcmp(px, qx, (size_t)LEN) == 0);
    ...
    canvas2d_clear_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.4f, 0.6f, 0.85f, 1.0f);
    canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_read_rgba(cv, CANVAS2D_CS_SRGB, px, LEN);
    canvas2d_clear_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_set_fill_linear_gradient(cv, CANVAS2D_CS_SRGB, CANVAS2D_ALPHA_UNPREMUL, 0.0f, 0.0f, (float)W, 0.0f);
    canvas2d_add_fill_color_stop(cv, CANVAS2D_CS_SRGB, 0.0f, 0.4f, 0.6f, 0.85f, 1.0f);
    canvas2d_add_fill_color_stop(cv, CANVAS2D_CS_SRGB, 1.0f, 0.4f, 0.6f, 0.85f, 1.0f);
    canvas2d_fill_rect(cv, 0.0f, 0.0f, (float)W, (float)H);
    canvas2d_read_rgba(cv, CANVAS2D_CS_SRGB, qx, LEN);
    CHECK(memcmp(px, qx, (size_t)LEN) == 0);
