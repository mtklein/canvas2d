...
static void test_variant_caps_substitutes(void) {
    ...
    canvas2d_set_font_family(cv, "Baskerville");
    canvas2d_set_font_size(cv, 34.0f);
    ...
    if (normal && small) {
        canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_NORMAL);
        render_text(cv, text, normal);
        canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_SMALL_CAPS);
...
static void test_variant_caps_noop(void) {
    ...
    if (normal && small) {
        canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_NORMAL);
        render_text(cv, text, normal);
        float const w_normal = canvas2d_measure_text(cv, text);
        canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_SMALL_CAPS);
        render_text(cv, text, small);
        CHECK(drew_anything(normal, NPX));
        CHECK(memcmp(normal, small, NPX) == 0);  // no smcp feature: identical
        CHECK(exact(canvas2d_measure_text(cv, text), w_normal));
        canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_ALL_SMALL_CAPS);
        CHECK(exact(canvas2d_measure_text(cv, text), w_normal));
        free(normal);
        free(small);
    }
...
static void test_stretch_condenses(void) {
    ...
    canvas2d_set_font_family(cv, "Futura");
    canvas2d_set_font_size(cv, 32.0f);
    char const *__null_terminated text = "Magazine";

    canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_NORMAL);
    float const w_normal = canvas2d_measure_text(cv, text);
    canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_CONDENSED);
    float const w_cond = canvas2d_measure_text(cv, text);
...
static void test_stretch_noop(void) {
    ...
    float const w_normal = canvas2d_measure_text(cv, text);
    canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_CONDENSED);
    CHECK(exact(canvas2d_measure_text(cv, text), w_normal));     // no width face
    canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_ULTRA_EXPANDED);
    CHECK(exact(canvas2d_measure_text(cv, text), w_normal));
    canvas2d_free(cv);
}
...
static void test_ignore_out_of_range(void) {
    ...
    canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_SMALL_CAPS);
    float const w_sc = canvas2d_measure_text(cv, text);
    canvas2d_set_font_variant_caps(cv, (enum canvas2d_font_variant_caps)999);  // ignored
    CHECK(exact(canvas2d_measure_text(cv, text), w_sc));

    // A garbage stretch enum is ignored: stretch stays CONDENSED.
    canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_NORMAL);
    canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_CONDENSED);
    float const w_cond = canvas2d_measure_text(cv, text);
    canvas2d_set_font_stretch(cv, (enum canvas2d_font_stretch)999);  // ignored
    CHECK(exact(canvas2d_measure_text(cv, text), w_cond));
    canvas2d_free(cv);
}
...
static void test_reset_restores_default(void) {
    ...
    float const w_default = canvas2d_measure_text(cv, text);  // NORMAL width
    canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_CONDENSED);
    CHECK(differ(canvas2d_measure_text(cv, text), w_default));

    canvas2d_reset(cv);
    canvas2d_set_font_family(cv, "Futura");  // reset cleared family + size too
    canvas2d_set_font_size(cv, 32.0f);
    CHECK(exact(canvas2d_measure_text(cv, text), w_default));  // back to NORMAL width
    canvas2d_free(cv);
}
...
static void draw_scene(struct canvas2d_context *__single cv) {
    ...
    canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_SMALL_CAPS);
    canvas2d_fill_text(cv, "Chapter", 6.0f, 30.0f);
    canvas2d_set_font_family(cv, "Futura");
    canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_NORMAL);
    canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_CONDENSED);
    canvas2d_fill_text(cv, "Magazine", 6.0f, 58.0f);
}
