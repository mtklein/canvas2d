...
static void flat_exact(void) {
    ...
    if (cv) {
        ...
        canvas_draw_image_scaled(cv, src, N, N, 4.0f, 4.0f, 8.0f, 8.0f);  // 4x minify
        ...
        canvas_draw_image_subrect(cv, src, N, N, 8.0f, 8.0f, 8.0f, 8.0f,
...
static void stripes_average(void) {
    ...
    if (cv) {
        canvas_set_image_smoothing_quality(cv, CANVAS_SMOOTHING_MEDIUM);
        canvas_draw_image_scaled(cv, src, N, N, 0.0f, 0.0f, 4.0f, 4.0f);
        canvas_read_rgba(cv, px, len);
...
static void catmullrom_phase(void) {
    ...
    if (cv) {
        canvas_set_image_smoothing_quality(cv, CANVAS_SMOOTHING_HIGH);
        canvas_draw_image_scaled(cv, src, 4, 4, 0.5f, 0.5f, 16.0f, 16.0f);
        canvas_read_rgba(cv, px, len);
...
static void no_transparent_bleed(void) {
    ...
    if (cv) {
        canvas_set_image_smoothing_quality(cv, CANVAS_SMOOTHING_HIGH);
        canvas_draw_image_scaled(cv, src, 2, 2, 0.0f, 0.0f, 16.0f, 16.0f);
        canvas_read_rgba(cv, px, len);
