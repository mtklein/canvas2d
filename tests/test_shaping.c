...
// The shaping seam is counted -- canvas2d_shape_text takes (bytes, len) pairs, never a NUL
...
static void check_shape(char const *__counted_by(len) text, int len, bool expect_rtl) {
    struct canvas2d_shaped *s = canvas2d_shape_text(S("Helvetica"), 20.0f, false, 400, false, 0, 0, 0, 4, S(""), text, len);
    CHECK(s != NULL);
    if (!s) {
        return;
    }
    ...
    float const w = canvas2d_shaped_width(s);
    CHECK(w > 0.0f);
    int const i0 = canvas2d_shaped_index_at_x(s, 0.0f);
    int const i1 = canvas2d_shaped_index_at_x(s, w * 0.99f);
    CHECK(i0 >= 0 && i0 < s->utf16s);
    CHECK(i1 >= 0 && i1 < s->utf16s);
    CHECK(canvas2d_shaped_index_at_x(s, w + 100.0f) == -1);  // past the end
...
static void check_fallback(void) {
    struct canvas2d_shaped *s = canvas2d_shape_text(S("Helvetica"), 20.0f, false, 400, false, 0, 0, 0, 4, S(""), S("A\xF0\x9F\x98\x80Z"));  // A 😀 Z
    CHECK(s != NULL);
    if (!s) {
        return;
    }
    char name0[128] = { 0 };
    int const n0 = canvas2d_run_font_name(s->run[0].font, name0, (int)sizeof name0);
    CHECK(n0 > 0);
    bool distinct = false;
    for (int r = 1; r < s->nruns; r++) {
        char nm[128] = { 0 };
        int const n = canvas2d_run_font_name(s->run[r].font, nm, (int)sizeof nm);
        CHECK(n > 0);
    ...
    canvas2d_run_font_name(s->run[0].font, guard, 4);
...
static void check_outline(void) {
    struct canvas2d_shaped *s = canvas2d_shape_text(S("Helvetica"), 40.0f, false, 400, false, 0, 0, 0, 4, S(""), S("ffi"));
    CHECK(s != NULL);
    if (s) {
    ...
    struct canvas2d_shaped *e = canvas2d_shape_text(S("Helvetica"), 40.0f, false, 400, false, 0, 0, 0, 4, S(""), S("\xF0\x9F\x98\x80"));  // lone emoji
    CHECK(e != NULL);
    if (e) {
        struct canvas2d_path p;
        canvas2d_path_init(&p);
...
static void check_emoji_draw(void) {
    struct canvas2d_shaped *s = canvas2d_shape_text(S("Helvetica"), 40.0f, false, 400, false, 0, 0, 0, 4, S(""), S("\xF0\x9F\x98\x80"));  // 😀
    CHECK(s != NULL);
    if (!s) {
        return;
    }
    ...
    if (s->nruns >= 1 && s->run[0].count >= 1) {
        ...
        CHECK(px != NULL);
        if (px) {
            canvas2d_glyph_draw(s->run[0].font, s->run[0].glyph[0], 12.0f, 12.0f, px, W, H);
...
static void check_bidi(void) {
    struct canvas2d_shaped *s = canvas2d_shape_text(S("Helvetica"), 20.0f, false, 400, false, 0, 0, 0, 4, S(""), S("Hi \xD7\xA9\xD7\x9C\xD7\x95\xD7\x9D!"));
    CHECK(s != NULL);
    if (!s) {
        return;
    }
    ...
    int const nfull = canvas2d_shaped_selection(s, 0, s->utf16s, sp, 8);
    CHECK(nfull == 1);
    ...
    int const nsplit = canvas2d_shaped_selection(s, 1, 5, sp, 8);
    CHECK(nsplit >= 2);
    ...
    CHECK(canvas2d_shaped_selection(s, 1, 5, sp, 1) <= 1);

    // Caret at the line start is 0; an index past the end is the line width.
    CHECK(canvas2d_shaped_x_at_index(s, 0) == 0.0f);
    float const xe = canvas2d_shaped_x_at_index(s, s->utf16s);
...
static void check_caret_snap(void) {
    ...
    float const w = canvas2d_shaped_width(s);
    float const at_emoji = canvas2d_shaped_x_at_index(s, 1);  // the pair's cluster start
    float const inside = canvas2d_shaped_x_at_index(s, 2);    // its low surrogate
    CHECK(inside - at_emoji == 0.0f);                // snapped to the cluster's edge
    CHECK(inside < w);                               // and NOT off the end of the line
    CHECK(canvas2d_shaped_x_at_index(s, 3) > at_emoji);  // 'z' sits past the emoji
    CHECK(canvas2d_shaped_x_at_index(s, 4) > w - 0.5f);  // end-of-line caret
    for (int i = 0; i <= s->utf16s; i++) {           // every caret lands on the line
        float const x = canvas2d_shaped_x_at_index(s, i);
...
static void check_paragraph_direction(void) {
    ...
    // right of the Hebrew.
    struct canvas2d_shaped *ltr = canvas2d_shape_text(S("Helvetica"), 20.0f, false, 400, false, 0, 0, 0, 4, S(""),
                                  S("Hi \xD7\xA9\xD7\x9C\xD7\x95\xD7\x9D!"));
    struct canvas2d_shaped *rtl = canvas2d_shape_text(S("Helvetica"), 20.0f, true, 400, false, 0, 0, 0, 4, S(""),
                                  S("Hi \xD7\xA9\xD7\x9C\xD7\x95\xD7\x9D!"));
    CHECK(ltr != NULL && rtl != NULL);
    if (ltr && rtl) {
        CHECK(canvas2d_shaped_x_at_index(ltr, 0) == 0.0f);
        CHECK(canvas2d_shaped_x_at_index(rtl, 0) > 0.0f);
        // Same glyphs in a different order: the advance widths agree (the sum
        // reorders, so allow the last-ulp wobble of float addition).
        float wl = canvas2d_shaped_width(ltr), wr = canvas2d_shaped_width(rtl);
        CHECK(wl > 0.0f);
        float const d = wl - wr;
        CHECK(d < 0.01f && d > -0.01f);
    }
    canvas2d_shaped_free(ltr);
    canvas2d_shaped_free(rtl);
    ...
    struct canvas2d_shaped *nl = canvas2d_shape_text(S("Helvetica"), 20.0f, false, 400, false, 0, 0, 0, 4, S(""),
                                 S("\xD7\xA9\xD7\x9C\xD7\x95\xD7\x9D!"));
    struct canvas2d_shaped *nr = canvas2d_shape_text(S("Helvetica"), 20.0f, true, 400, false, 0, 0, 0, 4, S(""),
                                 S("\xD7\xA9\xD7\x9C\xD7\x95\xD7\x9D!"));
    CHECK(nl != NULL && nr != NULL);
    if (nl && nr) {
        CHECK(canvas2d_shaped_x_at_index(nl, 4) > 0.0f);   // "!" at the right
        CHECK(canvas2d_shaped_x_at_index(nr, 4) == 0.0f);  // "!" at the left
    }
    canvas2d_shaped_free(nl);
    canvas2d_shaped_free(nr);
}
