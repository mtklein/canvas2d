...
// ink pixel count.
static long ink_bbox(struct canvas2d_context *__single cv, uint8_t *__counted_by(len) px, int len,
                     enum canvas2d_text_align align, enum canvas2d_text_baseline base,
                     float x0, float y0,
    ...
    canvas2d_set_text_baseline(cv, base);
    canvas2d_fill_text(cv, "Hi", x0, y0);
...
int main(void) {
    ...
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_LEFT, CANVAS2D_BASELINE_ALPHABETIC,
                   120.0f, 90.0f, &lx, &ly) > 0);
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_CENTER, CANVAS2D_BASELINE_ALPHABETIC,
                   120.0f, 90.0f, &cx, &cy) > 0);
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_RIGHT, CANVAS2D_BASELINE_ALPHABETIC,
    ...
    int sx, sy;
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_START, CANVAS2D_BASELINE_ALPHABETIC,
    ...
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_LEFT, CANVAS2D_BASELINE_ALPHABETIC,
                   20.0f, 80.0f, &ax, &ay) > 0);
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_LEFT, CANVAS2D_BASELINE_TOP,
                   20.0f, 80.0f, &tx, &ty) > 0);
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_LEFT, CANVAS2D_BASELINE_BOTTOM,
                   20.0f, 80.0f, &bx, &by) > 0);
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_LEFT, CANVAS2D_BASELINE_MIDDLE,
                   20.0f, 80.0f, &mx, &my) > 0);
    CHECK(ink_bbox(cv, px, len, CANVAS2D_ALIGN_LEFT, CANVAS2D_BASELINE_IDEOGRAPHIC,
                   20.0f, 80.0f, &ix, &iy) > 0);
