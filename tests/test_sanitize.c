...
int main(void) {
    ...
    for (int i = 0; i < nbad; i++) {
        float const v = bad[i];
        canvas2d_clear_rect(cv, v, v, v, v);                 // points_bbox (int) cast

        canvas2d_begin_path(cv);
        canvas2d_move_to(cv, v, 0.0f);
        canvas2d_line_to(cv, 0.0f, v);
        canvas2d_line_to(cv, v, v);
        canvas2d_fill(cv, CANVAS2D_NONZERO);                                   // coverage rasterizer casts

        canvas2d_begin_path(cv);
        canvas2d_ellipse(cv, 0.0f, 0.0f, 10.0f, 10.0f, 0.0f, 0.0f, v, false);
        canvas2d_fill(cv, CANVAS2D_NONZERO);                                   // ellipse segment-count cast

        canvas2d_set_line_width(cv, v);
        canvas2d_begin_path(cv);
        canvas2d_move_to(cv, 0.0f, 0.0f);
        canvas2d_line_to(cv, 20.0f, 20.0f);
        canvas2d_stroke(cv);                                 // stroke segment-count cast

        canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, v, v, v, v);              // colour clamp -> [0,1]
        canvas2d_set_global_alpha(cv, v);
    }
    ...
    canvas2d_set_line_dash(cv, tiny_dash, 2);
    canvas2d_begin_path(cv);
    canvas2d_move_to(cv, 0.0f, 0.0f);
    canvas2d_line_to(cv, 1.0e6f, 1.0e6f);
    canvas2d_stroke(cv);
    canvas2d_set_line_dash(cv, NULL, 0);  // back to solid
    ...
    canvas2d_fill_rect(cv, 0.0f, 0.0f, 64.0f, 64.0f);

    uint8_t px[64 * 64 * 4];
    canvas2d_get_image_data(cv, CANVAS2D_CS_SRGB, 0, 0, 64, 64, px, (int)sizeof px);
