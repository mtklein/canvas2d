...
// The public selection/caret queries (canvas_text_index_at_x /
// canvas_text_x_at_index / canvas_text_selection) wrap the shaped-line logic
...
int main(void) {
    ...
    float const w = canvas_measure_text(cv, txt);
    CHECK(w > 0.0f);

    // Caret at index 0 is the start; at/past the end is the advance width.
    CHECK(canvas_text_x_at_index(cv, txt, 0) == 0.0f);
    float const x_end = canvas_text_x_at_index(cv, txt, (int)strlen(txt));
    CHECK(fabsf(x_end - w) < 0.5f);
    // Past the end clamps to the same end caret.
    CHECK(fabsf(canvas_text_x_at_index(cv, txt, 999) - w) < 0.5f);

    // Caret x is monotonic non-decreasing across logical indices (LTR text).
    float prev = -1.0f;
    for (int i = 0; i <= (int)strlen(txt); i++) {
        float const xi = canvas_text_x_at_index(cv, txt, i);
    ...
    for (int i = 0; i < (int)strlen(txt); i++) {
        float const xi = canvas_text_x_at_index(cv, txt, i);
        int const hit = canvas_text_index_at_x(cv, txt, xi + 0.1f);
        CHECK(hit == i);
    }

    // A visual x far past the line's end maps to no index.
    CHECK(canvas_text_index_at_x(cv, txt, w + 100.0f) == -1);

    // Full-range selection collapses to one span covering ~the whole width.
    canvas_text_span sp[8];
    int const nfull = canvas_text_selection(cv, txt, 0, (int)strlen(txt), sp, 8);
    CHECK(nfull == 1);
    CHECK(sp[0].x0 == 0.0f);
    CHECK(fabsf(sp[0].x1 - w) < 0.5f);

    // A sub-range yields >= 1 span, each within [0, width] and ordered.
    int const nsub = canvas_text_selection(cv, txt, 1, 3, sp, 8);
    CHECK(nsub >= 1);
    ...
    float const mid_no_ls = canvas_text_x_at_index(cv, txt, 3);
    canvas_set_letter_spacing(cv, 5.0f);
    float const mid_ls = canvas_text_x_at_index(cv, txt, 3);
    CHECK(mid_ls > mid_no_ls);
    // And the full width grows with it.
    CHECK(canvas_measure_text(cv, txt) > w);
    canvas_set_letter_spacing(cv, 0.0f);

    // Edges: empty text, max <= 0, NULL out -> no crash, sensible empties.
    CHECK(canvas_text_x_at_index(cv, "", 0) == 0.0f);
    CHECK(canvas_text_x_at_index(cv, "", 5) == 0.0f);
    CHECK(canvas_text_index_at_x(cv, "", 0.0f) == -1
       || canvas_text_index_at_x(cv, "", 0.0f) == 0);  // empty line: no glyphs to hit
    CHECK(canvas_text_selection(cv, "", 0, 0, sp, 8) == 0);
    CHECK(canvas_text_selection(cv, txt, 0, (int)strlen(txt), sp, 0) == 0);
    ...
    canvas_text_span *__counted_by(0) none = 0;  // a NULL buffer with count 0
    CHECK(canvas_text_selection(cv, txt, 0, (int)strlen(txt), none, 0) == 0);
    // An inverted range selects nothing.
    CHECK(canvas_text_selection(cv, txt, 3, 1, sp, 8) == 0);
    ...
    float const wb = canvas_measure_text(cv, bidi);
    CHECK(wb > 0.0f);
    int const ub = (int)strlen(bidi);  // an upper bound on the UTF-16 length
    canvas_text_span bsp[16];
    int const nb = canvas_text_selection(cv, bidi, 1, 5, bsp, 16);
    CHECK(nb >= 1);
    for (int k = 0; k < nb; k++) {
    ...
    for (int i = 0; i <= ub; i++) {
        float const xi = canvas_text_x_at_index(cv, bidi, i);
