// The params -> derived-data text lookup (struct canvas2d_text_cache in src/canvas2d_text.h):
...
// canvas2d_canvas_text_cache handle -- they are instrumentation, not public API.
...
static void draw_scene(struct canvas2d_context *__single cv) {
    canvas2d_set_font_size(cv, 18.0f);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.10f, 0.20f, 0.30f, 1.0f);
    canvas2d_fill_text(cv, "Waffle fan", 4.0f, 22.0f);
    canvas2d_set_fill_rgba(cv, CANVAS2D_CS_SRGB, 0.80f, 0.30f, 0.20f, 1.0f);
    canvas2d_fill_text(cv, "A\xF0\x9F\x98\x80g", 4.0f, 44.0f);  // A 😀 g
}
...
static void check_transparent(void) {
    struct canvas2d_context *__single cold = canvas2d(W, H, CANVAS2D_CS_SRGB);
    struct canvas2d_context *__single warm = canvas2d(W, H, CANVAS2D_CS_SRGB);
    ...
    canvas2d_clear_rect(warm, 0.0f, 0.0f, (float)W, (float)H);
    struct canvas2d_text_cache *__single c = canvas2d_canvas_text_cache(warm);
    ...
    uint8_t a[LEN], b[LEN];
    canvas2d_get_image_data(cold, CANVAS2D_CS_SRGB, 0, 0, W, H, a, LEN);
    canvas2d_get_image_data(warm, CANVAS2D_CS_SRGB, 0, 0, W, H, b, LEN);
...
static void check_keys(void) {
    ...
    canvas2d_fill_text(cv, "kerning", 4.0f, 30.0f);
    CHECK(c->shaping_misses == 1 && c->shaping_hits == 0);
    CHECK(canvas2d_measure_text(cv, "kerning") > 0.0f);  // measure after draw: hit
    CHECK(c->shaping_misses == 1 && c->shaping_hits == 1);
    (void)canvas2d_measure_text_full(cv, "kerning");     // the full metrics too
    CHECK(c->shaping_misses == 1 && c->shaping_hits == 2);

    canvas2d_set_font_size(cv, 21.0f);                   // same bytes, other size
    (void)canvas2d_measure_text(cv, "kerning");
    CHECK(c->shaping_misses == 2 && c->shaping_hits == 2);

    canvas2d_set_font_size(cv, 20.0f);                   // same size, other bytes
    (void)canvas2d_measure_text(cv, "kerninG");
    CHECK(c->shaping_misses == 3 && c->shaping_hits == 2);

    (void)canvas2d_measure_text(cv, "kerning");          // the original is still hot
    ...
    (void)canvas2d_measure_text(cv, "kerning");          // the rtl line is hot...
    CHECK(c->shaping_misses == 4 && c->shaping_hits == 4);
    canvas2d_set_direction(cv, CANVAS2D_DIRECTION_LTR);
    (void)canvas2d_measure_text(cv, "kerning");          // ...and the ltr one kept
...
// counted seam (the same lookup canvas2d_measure_text lands on, same stats, same
...
static void check_eviction(void) {
    struct canvas2d_context *__single churn = canvas2d(W, H, CANVAS2D_CS_SRGB);
    ...
    struct canvas2d_text_cache *__single c = canvas2d_canvas_text_cache(churn);
    canvas2d_set_font_size(churn, 16.0f);

    float const w0 = canvas2d_measure_text(churn, "s0");
    char last[3] = { 0 };
    for (int i = 1; i <= CANVAS2D_SHAPING_CACHE_N; i++) {  // fill every slot, plus one
        last[0] = 's';                               // distinct two-letter keys
        last[1] = (char)('a' + i / 26);
        last[2] = (char)('a' + i % 26);
        (void)canvas2d_text_cache_shaping(c, k_family, (int)sizeof k_family - 1,
                                    16.0f, false, 0.0f, 0.0f, 400, false, 0, 0, 0, 4, "", 0,
                                    last, (int)sizeof last);
    }
    CHECK(c->shaping_misses == CANVAS2D_SHAPING_CACHE_N + 1);

    int const hits = c->shaping_hits;
    // Evicted: re-shaped from the boundary, to the bit-identical width.
    CHECK(fabsf(canvas2d_measure_text(churn, "s0") - w0) <= 0.0f);
    CHECK(c->shaping_misses == CANVAS2D_SHAPING_CACHE_N + 2);
    ...
    canvas2d_set_fill_rgba(churn, CANVAS2D_CS_SRGB, 0.2f, 0.2f, 0.7f, 1.0f);
    canvas2d_fill_text(churn, "s0", 4.0f, 30.0f);
    canvas2d_set_font_size(fresh, 16.0f);
    canvas2d_set_fill_rgba(fresh, CANVAS2D_CS_SRGB, 0.2f, 0.2f, 0.7f, 1.0f);
    canvas2d_fill_text(fresh, "s0", 4.0f, 30.0f);
    uint8_t a[LEN], b[LEN];
    canvas2d_get_image_data(churn, CANVAS2D_CS_SRGB, 0, 0, W, H, a, LEN);
    canvas2d_get_image_data(fresh, CANVAS2D_CS_SRGB, 0, 0, W, H, b, LEN);
    CHECK(memcmp(a, b, LEN) == 0);
...
static void check_glyph_once(void) {
    ...
    canvas2d_fill_text(cv, "AA AA", 2.0f, 30.0f);  // two distinct glyphs: 'A', ' '
    CHECK(c->glyph_misses == 2);
    CHECK(c->glyph_hits == 3);
    CHECK(c->glyph_count == 2);
    CHECK(c->nfonts == 1);

    canvas2d_fill_text(cv, "AA AA", 2.0f, 30.0f);  // warm: no new fetches
    CHECK(c->glyph_misses == 2);
...
static void check_reset(void) {
    ...
    struct canvas2d_text_cache *__single c = canvas2d_canvas_text_cache(cv);
    canvas2d_set_font_size(cv, 18.0f);
    canvas2d_fill_text(cv, "Reset", 4.0f, 30.0f);
    ...
    canvas2d_set_font_size(cv, 18.0f);  // reset dropped the 18px state too
    canvas2d_fill_text(cv, "Reset", 4.0f, 30.0f);
    CHECK(c->shaping_misses == 1);      // cold again

    canvas2d_set_font_size(fresh, 18.0f);
    canvas2d_fill_text(fresh, "Reset", 4.0f, 30.0f);
    uint8_t a[LEN], b[LEN];
