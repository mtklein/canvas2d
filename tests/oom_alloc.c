...
// -Dmalloc=canvas2d_oom_malloc redefines, so malloc/realloc/calloc below are the real
...
void *canvas2d_oom_malloc(size_t size) {
    return trip() ? NULL : malloc(size);
}

void *canvas2d_oom_realloc(void *p, size_t size) {
...
void *canvas2d_oom_calloc(size_t n, size_t size) {
