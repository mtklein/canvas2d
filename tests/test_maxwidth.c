...
static void ink_extent(struct canvas2d_context *__single cv, uint8_t *__counted_by(len) px, int len,
...
static void clear_white(struct canvas2d_context *__single cv) {
...
int main(void) {
    ...
    float const adv = canvas2d_measure_text(cv, s);
    CHECK(adv > 0.0f);

    // Baseline: unconstrained render.
    clear_white(cv);
    canvas2d_fill_text(cv, s, 10.0f, 55.0f);
    int wn, hn;
    ...
    clear_white(cv);
    canvas2d_fill_text_max(cv, s, 10.0f, 55.0f, adv * 0.5f);
    int wh, hh;
    ...
    clear_white(cv);
    canvas2d_fill_text_max(cv, s, 10.0f, 55.0f, adv * 2.0f);
    int ww, hw;
    ...
    clear_white(cv);
    canvas2d_fill_text_max(cv, s, 10.0f, 55.0f, 0.0f);
    int wz, hz;
    ...
    canvas2d_stroke_text_max(cv, s, 10.0f, 55.0f, adv * 0.5f);
    int ws, hs;
