...
static bool finite3(cnvs_rgb c) {
    return isfinite(c.r) && isfinite(c.g) && isfinite(c.b);
}

static bool finite_lab(cnvs_oklab c) {
...
int main(void) {
    ...
    for (int code = 0; code < 256; code++) {
        ...
        float const back = cnvs_linear_to_srgb(cnvs_srgb_to_linear(c));
    ...
    for (int i = 0; i <= 100; i++) {
        ...
        CHECK(near(cnvs_srgb_to_linear(cnvs_linear_to_srgb(l)), l, 1e-5f));
    }
    ...
    for (int i = 0; i < N; i++) {
        CHECK(isfinite(cnvs_srgb_to_linear(spread[i])));
        CHECK(isfinite(cnvs_linear_to_srgb(spread[i])));
    }
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            for (int k = 0; k < N; k++) {
                cnvs_rgb const lin = { spread[i], spread[j], spread[k] };
                cnvs_oklab const lab = cnvs_linear_srgb_to_oklab(lin);
                CHECK(finite_lab(lab));
                CHECK(finite3(cnvs_oklab_to_linear_srgb(lab)));
            }
        }
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        CHECK(near(cnvs_srgb_to_linear(-x), -cnvs_srgb_to_linear(x), 1e-7f));
        CHECK(near(cnvs_linear_to_srgb(-x), -cnvs_linear_to_srgb(x), 1e-7f));
    }

    // --- Oklab known values (Ottosson's reference numbers) --------------------
    cnvs_oklab const white = cnvs_linear_srgb_to_oklab((cnvs_rgb){ 1.0f, 1.0f, 1.0f });
    CHECK(near(white.L, 1.0f, 1e-4f));
    CHECK(near(white.a, 0.0f, 1e-4f));
    CHECK(near(white.b, 0.0f, 1e-4f));

    cnvs_oklab const black = cnvs_linear_srgb_to_oklab((cnvs_rgb){ 0.0f, 0.0f, 0.0f });
    ...
    cnvs_oklab const red = cnvs_linear_srgb_to_oklab((cnvs_rgb){ 1.0f, 0.0f, 0.0f });
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < M; j++) {
            for (int k = 0; k < M; k++) {
                cnvs_rgb const lin = { grid[i], grid[j], grid[k] };
                cnvs_rgb const rt = cnvs_oklab_to_linear_srgb(
                                        cnvs_linear_srgb_to_oklab(lin));
    ...
    cnvs_rgb const enc = { 0.1f, 0.5f, 0.9f };
    cnvs_rgb const dec = cnvs_rgb_srgb_to_linear(enc);
    CHECK(near(dec.r, cnvs_srgb_to_linear(enc.r), 0.0f));
    CHECK(near(dec.g, cnvs_srgb_to_linear(enc.g), 0.0f));
    CHECK(near(dec.b, cnvs_srgb_to_linear(enc.b), 0.0f));
    cnvs_rgb const reenc = cnvs_rgb_linear_to_srgb(dec);
