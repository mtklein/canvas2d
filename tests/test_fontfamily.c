...
#include "canvas.h"
#include "cnvs_text.h"  // the cache handle: resolved font names + stats
...
static void test_distinct_families(void) {
    ...
    float const w_helv = canvas_measure_text(cv, text);
    canvas_set_font_family(cv, "Georgia");
    float const w_geo = canvas_measure_text(cv, text);
    canvas_set_font_family(cv, "Menlo");  // monospace: a third distinct metric
    float const w_mono = canvas_measure_text(cv, text);
    ...
    canvas_set_font_family(cv, "Helvetica");
    (void)canvas_measure_text(cv, text);  // already shaped above: a hit
...
static void test_bogus_family(void) {
    ...
    canvas_set_font_family_n(cv, bogus, blen);
    canvas_set_font_size(cv, 28.0f);

    float const w = canvas_measure_text(cv, "Hi");
    CHECK(w > 0.0f);  // the fallback cascade produced a measurable line

    canvas_fill_text(cv, "Hi", 4.0f, 36.0f);

    uint8_t *__counted_by(NPX) px = malloc(NPX);
    CHECK(px != NULL);
    if (px) {
        canvas_read_rgba(cv, CANVAS_CS_SRGB, px, NPX);
...
static void test_reset_restores_default(void) {
    ...
    float const w_default = canvas_measure_text(cv, text);  // Libian TC
    canvas_set_font_family(cv, "Menlo");
    float const w_menlo = canvas_measure_text(cv, text);
    CHECK(differ(w_default, w_menlo));  // the family changed the measurement

    canvas_reset(cv);
    canvas_set_font_size(cv, 24.0f);  // reset also cleared the size
    float const w_after = canvas_measure_text(cv, text);
...
static void test_null_empty_ignored(void) {
    ...
    canvas_set_font_family(cv, NULL);  // ignored
    CHECK(exact(canvas_measure_text(cv, text), w_menlo));
    canvas_set_font_family(cv, "");    // ignored
    CHECK(exact(canvas_measure_text(cv, text), w_menlo));
...
static void draw_scene(struct canvas *__single cv) {
    ...
    canvas_fill_text(cv, "Hi 字", 4.0f, 32.0f);
}
