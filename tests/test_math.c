...
int main(void) {
    ...
    CHECK(vnear(canvas2d_mat_apply(t, (canvas2d_vec2){ .x = 1.0f, .y = 1.0f }), 11.0f, -4.0f));

    canvas2d_mat const s = canvas2d_mat_scale(2.0f, 3.0f);
    CHECK(vnear(canvas2d_mat_apply(s, (canvas2d_vec2){ .x = 4.0f, .y = 4.0f }), 8.0f, 12.0f));
    ...
    CHECK(vnear(canvas2d_mat_apply(ts, (canvas2d_vec2){ .x = 1.0f, .y = 1.0f }), 12.0f, -2.0f));

    // Rotate +90 degrees: (1,0) -> (0,1) in a +y-down frame.
    canvas2d_mat const r = canvas2d_mat_rotate((float)M_PI / 2.0f);
    CHECK(vnear(canvas2d_mat_apply(r, (canvas2d_vec2){ .x = 1.0f, .y = 0.0f }), 0.0f, 1.0f));
