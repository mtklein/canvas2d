...
// reaches because it only ever encodes valid canvas sizes.  cnvs_png_encode
...
static void rejects_bad_dimensions(void) {
    ...
    int len;
    len = -1; CHECK(cnvs_png_encode(buf, 0, 1, &len) == NULL && len == 0);
    len = -1; CHECK(cnvs_png_encode(buf, 1, 0, &len) == NULL && len == 0);
    len = -1; CHECK(cnvs_png_encode(buf, 16385, 1, &len) == NULL && len == 0);
    len = -1; CHECK(cnvs_png_encode(buf, 1, 16385, &len) == NULL && len == 0);
    free(buf);
}
...
static void encodes_valid_png(void) {
    uint16_t px[2 * 2 * 4] = {0};
    int len = 0;
    uint8_t *a = cnvs_png_encode(px, 2, 2, &len);
    CHECK(a != NULL);
    CHECK(len > 8);
    if (a && len >= 8) {
    ...
    int len2 = 0;
    uint8_t *b = cnvs_png_encode(px, 2, 2, &len2);
