...
**The gradient ramp.** `cnvs_gradient.h` documents ≤0.4/255 max deviation from the exact ramp, "below the 8-bit step." f16 entries add ≤2⁻¹¹ ≈ 0.125/255 near 1.0 — inside the budget. u8 entries would add up to 0.5/255 and exceed the documented claim. Low-alpha stops also premultiply through the ramp; xp2 shows f16 holds them exactly where u8 wouldn't.
