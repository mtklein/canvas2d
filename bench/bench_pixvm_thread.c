// Isolated benchmark: the tail-call threaded pixel VM (design B), same source-over
// program and tile as bench_pixvm_switch.c -- so the two times isolate dispatch
// style (switch vs musttail threading) at equal pixel I/O and arithmetic.
...
int main(void) {
    ...
    for (int rep = 0; rep < reps; rep++) {
        for (int it = 0; it < ITERS; it++) {
            pixvm_run_threaded(prog, nops, px, NULL, cov, n);
