...
// boundaries.  The bulk kernels are PLANAR over that type (canvas2d_planar.h);
...
// canvas2d_unpremul is what the Canvas API speaks (r,g,b independent of a); canvas2d_premul
// is what internal pixel buffers hold (r,g,b scaled by a).  Convert only through
// canvas2d_premultiply / canvas2d_unpremultiply, never by reinterpreting the shared layout.
typedef struct canvas2d_unpremul {
    _Float16 r, g, b, a;
} canvas2d_unpremul;

typedef struct canvas2d_premul {
    _Float16 r, g, b, a;
} canvas2d_premul;
...
// [0,1] (PQ has no meaning outside it).  canvas2d_pq_oetf(0)=0, (1)=1, monotone.
float canvas2d_pq_oetf(float y);

// 8-wide canvas2d_pq_oetf: lane-for-lane bit-identical to the scalar (mirrors its
