...
// signalled by a cICP chunk (the canvas2d_png encoder writes the chunk; this is the
...
#define CANVAS2D_REF_WHITE_NITS 203.0f
...
static void surface_to_pq16(struct canvas2d_context *__single cv,
                            uint16_t *__counted_by(n4) out, int n4) {
    (void)n4;
    bool const lin = cv->space == CANVAS2D_CS_LINEAR_SRGB;
    float const scale = CANVAS2D_REF_WHITE_NITS / 10000.0f;
    ...
    for (; i + 8 <= n; i += 8) {
        ...
        for (int j = 0; j < 8; j++) {
            ...
            if (a > 0.0f) {
                ...
                canvas2d_rgb l = { (float)px.r * inv, (float)px.g * inv, (float)px.b * inv };
                if (!lin) {
                    l = canvas2d_rgb_srgb_to_linear(l);
                }
                canvas2d_rgb const wide = canvas2d_linear_srgb_to_rec2020(l);
                wr[j] = wide.r * scale;
                wg[j] = wide.g * scale;
                wb[j] = wide.b * scale;
            }
    ...
    for (; i < n; i++) {  // scalar tail (n % 8), the original per-pixel path
        canvas2d_premul const px = cv->target[i];
        float const a = (float)px.a;
        ...
        if (a > 0.0f) {  // a transparent pixel stays all-zero
            ...
            canvas2d_rgb l = { (float)px.r * inv, (float)px.g * inv, (float)px.b * inv };
            if (!lin) {
                l = canvas2d_rgb_srgb_to_linear(l);  // sRGB surface -> linear light
            }
            canvas2d_rgb const wide = canvas2d_linear_srgb_to_rec2020(l);
            r  = q16(canvas2d_pq_oetf(wide.r * scale));
            g  = q16(canvas2d_pq_oetf(wide.g * scale));
            b  = q16(canvas2d_pq_oetf(wide.b * scale));
            av = q16(a);
        }
        out[i * 4 + 0] = r;
        out[i * 4 + 1] = g;
...
// The in-memory sibling of canvas2d_write_png; the byte gate uses it to compare
...
canvas2d_encode_png(struct canvas2d_context *__single cv, int *__single outlen) {
    *outlen = 0;
    ...
    uint8_t *out = canvas2d_png_encode(px, cv->width, cv->height, outlen);
    free(px);
    return out;
}
...
bool canvas2d_write_png(struct canvas2d_context *__single cv, char const *__null_terminated path) {
    int len = 0;
    uint8_t *out = canvas2d_encode_png(cv, &len);
    if (!out) {
        return false;
    }
    bool ok = false;
