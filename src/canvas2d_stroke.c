...
static bool seg_dir(canvas2d_vec2 p0, canvas2d_vec2 p1, canvas2d_vec2 *dir, float *len) {
    float const dx = p1.x - p0.x;
    float const dy = p1.y - p0.y;
    float const l = sqrtf(dx * dx + dy * dy);
    *dir = (canvas2d_vec2){ .x = dx / l, .y = dy / l };
    *len = l;
    return !(l < 1e-6f);
}

// The quad's two triangles, staged: (a0,b0,b1) (a0,b1,a1).
static void stage_quad(canvas2d_vec2 *__counted_by(6) stage, canvas2d_vec2 a0, canvas2d_vec2 b0,
                       canvas2d_vec2 a1, canvas2d_vec2 b1) {
    stage[0] = a0;
    stage[1] = b0;
    stage[2] = b1;
    stage[3] = a0;
...
static bool emit_quad(struct canvas2d_verts *out, canvas2d_vec2 p0, canvas2d_vec2 p1, canvas2d_vec2 nrm) {
    canvas2d_vec2 const a0 = { .x = p0.x + nrm.x, .y = p0.y + nrm.y };
    canvas2d_vec2 const b0 = { .x = p0.x - nrm.x, .y = p0.y - nrm.y };
    canvas2d_vec2 const a1 = { .x = p1.x + nrm.x, .y = p1.y + nrm.y };
    canvas2d_vec2 const b1 = { .x = p1.x - nrm.x, .y = p1.y - nrm.y };
    canvas2d_vec2 stage[6];
    stage_quad(stage, a0, b0, a1, b1);
    return canvas2d_verts_append(out, stage, 6);
}
...
static bool emit_disc(struct canvas2d_verts *out, canvas2d_vec2 c, float r) {
    int const segs = disc_segs(r);
    canvas2d_vec2 stage[3 * 64];
    float const step = TAU / (float)segs;
    float const cs = cosf(step), sn = sinf(step);
    float dx = r, dy = 0.0f;
    canvas2d_vec2 prev = { .x = c.x + r, .y = c.y };
    int k = 0;
    for (int i = 1; i <= segs; i++) {
        ...
        dx = nx;
        dy = ny;
        canvas2d_vec2 const cur = { .x = c.x + dx, .y = c.y + dy };
        canvas2d_vec2 *__counted_by(3) tri = stage + k;
        tri[0] = c;
        tri[1] = prev;
        tri[2] = cur;
        k += 3;
    ...
    return canvas2d_verts_append(out, stage, k);
}
...
static int stage_wedge(canvas2d_vec2 *__counted_by(6) stage, canvas2d_vec2 v, canvas2d_vec2 d0,
                       canvas2d_vec2 d1, float cross, float half_width,
                       enum canvas2d_line_join join, float miter_limit) {
    // Outer side is opposite the turn.
    float const sgn = cross > 0.0f ? -1.0f : 1.0f;
    canvas2d_vec2 pa = { .x = v.x + sgn * -d0.y * half_width,
                     .y = v.y + sgn * d0.x * half_width };
    canvas2d_vec2 pb = { .x = v.x + sgn * -d1.y * half_width,
                     .y = v.y + sgn * d1.x * half_width };
    stage[0] = pa;  // bevel wedge
    stage[1] = v;
    stage[2] = pb;
    if (join == CANVAS2D_JOIN_MITER) {
        // Miter tip = intersection of the two outer edges (pa,d0) and (pb,d1).
        float const s = ((pb.x - pa.x) * d1.y - (pb.y - pa.y) * d1.x) / cross;
        canvas2d_vec2 const tip = { .x = pa.x + d0.x * s, .y = pa.y + d0.y * s };
...
static int join_at(struct canvas2d_verts *out, canvas2d_vec2 *__counted_by(48) stage, int k,
                   canvas2d_vec2 v, canvas2d_vec2 d0, canvas2d_vec2 d1, float half_width,
                   enum canvas2d_line_join join, float miter_limit) {
    ...
    if (join == CANVAS2D_JOIN_ROUND) {
        if (!canvas2d_verts_append(out, stage, k) || !emit_disc(out, v, half_width)) {
            return -1;
        }
        return 0;
    }
...
static bool emit_join(struct canvas2d_verts *out, canvas2d_vec2 v, canvas2d_vec2 d0, canvas2d_vec2 d1,
                      float half_width, enum canvas2d_line_join join, float miter_limit) {
    canvas2d_vec2 stage[48];
    int const k = join_at(out, stage, 0, v, d0, d1, half_width, join, miter_limit);
    return k >= 0 && canvas2d_verts_append(out, stage, k);
}

// Cap at open end `e`, with `capdir` pointing outward along the line.
static bool emit_cap(struct canvas2d_verts *out, canvas2d_vec2 e, canvas2d_vec2 capdir,
                     float half_width, enum canvas2d_line_cap cap) {
    if (cap == CANVAS2D_CAP_BUTT) {
        return true;
    }
    if (cap == CANVAS2D_CAP_ROUND) {
        return emit_disc(out, e, half_width);
    }
    canvas2d_vec2 const nrm = { .x = -capdir.y * half_width, .y = capdir.x * half_width };
    canvas2d_vec2 const e2 = { .x = e.x + capdir.x * half_width, .y = e.y + capdir.y * half_width };
...
bool canvas2d_stroke_polyline(canvas2d_vec2 const *__counted_by(n) pts, int n, bool closed,
    ...
    canvas2d_vec2 prev_dir = { .x = 0.0f, .y = 0.0f };
    canvas2d_vec2 first_dir = { .x = 0.0f, .y = 0.0f };
    canvas2d_vec2 first_pt = { .x = 0.0f, .y = 0.0f };
    canvas2d_vec2 last_pt = { .x = 0.0f, .y = 0.0f };
...
#define CANVAS2D_PUT_WEDGE(i)                                                        \
    do {                                                                         \
        ...
        memcpy(wedges + 6 * (i) + 4, &t2, sizeof t2);                            \
                ...
                CANVAS2D_PUT_WEDGE(0);
                CANVAS2D_PUT_WEDGE(1);
                CANVAS2D_PUT_WEDGE(2);
                CANVAS2D_PUT_WEDGE(3);
#undef CANVAS2D_PUT_WEDGE
                memcpy(col4, &col, sizeof col4);
                memcpy(ok4, &ok, sizeof ok4);
            }

            canvas2d_vec2 stage[48];  // 4 segments x (6 quad verts + 6 join verts)
            int k = 0;
            for (int i = 0; i < 4; i++) {
                if (l4[i] < 1e-6f) {
                ...
                k += 6;
                canvas2d_vec2 dir = { .x = dx4[i], .y = dy4[i] };
                if (have_prev) {
                    if (vjoin) {
                        if (!col4[i]) {  // collinear: no gap to fill
                            if (join == CANVAS2D_JOIN_MITER && ok4[i]) {
            ...
            if (!canvas2d_verts_append(out, stage, k)) {
                return false;
            }
            s += 4;
        } else {
            canvas2d_vec2 const p0 = pts[s];
            canvas2d_vec2 const p1 = pts[(s + 1) % m];
            canvas2d_vec2 dir;
            float len;
            if (seg_dir(p0, p1, &dir, &len)) {
                canvas2d_vec2 const nrm = { .x = -dir.y * half_width, .y = dir.x * half_width };
    ...
    canvas2d_vec2 const back = { .x = -first_dir.x, .y = -first_dir.y };
...
bool canvas2d_stroke_dashed(canvas2d_vec2 const *__counted_by(n) pts, int n, bool closed,
    ...
    for (int s = 0; s < nseg; s++) {
        canvas2d_vec2 const a = pts[s];
        canvas2d_vec2 const b = pts[(s + 1) % n];
        canvas2d_vec2 dir;
        float seglen;
        ...
        while (pos < seglen) {
            ...
            if (on) {
                canvas2d_vec2 const p0 = { .x = a.x + dir.x * pos, .y = a.y + dir.y * pos };
                canvas2d_vec2 p1 = { .x = a.x + dir.x * (pos + step),
