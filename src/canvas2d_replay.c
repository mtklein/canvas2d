// Text canvas-program parser + replayer (the canvas2d_replay.h core, and
// canvas2d_replay_from()).  Parses untrusted text by index over a __counted_by
// buffer -- the bounds-safe-parsing exercise -- and dispatches to the public
// canvas2d_* API.  No forges and no __null_terminated: numbers are parsed in place
// by index (no strtof), and the text tail is passed as a slice to the
// length-counted canvas2d_*_text_n; the parser stays entirely in the indexable
// world.  (docs/bounds-safety.md walks through what that took.)

#include "canvas2d_replay.h"

#include "canvas2d.h"
#include "canvas2d_path2d_internal.h"  // struct canvas2d_path2d's command count (OOM-drop detection)
#include "canvas2d_record.h"  // the shared numbered-object caps (CANVAS2D_REC_*_MAX)
...
#define REPLAY_BITMAP_DIM_MAX 512    // capture dims cap: bounds a bitmap
                                     ...
                                     // (the recorder writes CANVAS2D_CAPTURE_EM =
                                     // 160); the deflated stream is further
                                     // capped at canvas2d_zlib_bound of that, so
...
// A colour op line's REQUIRED trailing colour-space token, read after its
// floats: one of the three colour-space names (the same token canvas2d_rec_floats_cs
// writes unconditionally).  sRGB, linear-sRGB, and Oklab are equal peers, so the
// token is mandatory -- a missing token (end of line) is a malformed file, not
// an sRGB default.  Strict: any other token, no token at all, or anything after
// the token is malformed and stops replay, matching the format's posture for
// every named enum.  *jp is advanced past the token.
static bool read_cs(char const *__counted_by(le) data, size_t le,
                    size_t *__single jp, enum canvas2d_color_space *__single out) {
    size_t ts, tl;
    if (!read_token(data, le, jp, &ts, &tl)) {
        return false;  // no token: the space is required
    }
    if      (tok_eq(data, le, ts, tl, canvas2d_color_space_name[CANVAS2D_CS_SRGB])) {
        *out = CANVAS2D_CS_SRGB;
    } else if (tok_eq(data, le, ts, tl, canvas2d_color_space_name[CANVAS2D_CS_LINEAR_SRGB])) {
        *out = CANVAS2D_CS_LINEAR_SRGB;
    } else if (tok_eq(data, le, ts, tl, canvas2d_color_space_name[CANVAS2D_CS_OKLAB])) {
        *out = CANVAS2D_CS_OKLAB;
    } else {
...
static bool read_interp(char const *__counted_by(le) data, size_t le,
                        size_t *__single jp, enum canvas2d_color_space *__single space,
                        enum canvas2d_alpha_type *__single alpha) {
    size_t ts, tl;
    if (!read_token(data, le, jp, &ts, &tl)) {
        return false;
    }
    if      (tok_eq(data, le, ts, tl, canvas2d_color_space_name[CANVAS2D_CS_SRGB])) {
        ...
        *space = CANVAS2D_CS_LINEAR_SRGB;
    } else if (tok_eq(data, le, ts, tl, canvas2d_color_space_name[CANVAS2D_CS_OKLAB])) {
        *space = CANVAS2D_CS_OKLAB;
    } else {
        return false;
    }
    if (!read_token(data, le, jp, &ts, &tl)) {
    ...
    if      (tok_eq(data, le, ts, tl, canvas2d_alpha_type_name[CANVAS2D_ALPHA_UNPREMUL])) {
        *alpha = CANVAS2D_ALPHA_UNPREMUL;
    } else if (tok_eq(data, le, ts, tl, canvas2d_alpha_type_name[CANVAS2D_ALPHA_PREMUL])) {
        *alpha = CANVAS2D_ALPHA_PREMUL;
    } else {
        return false;
    }
    return true;
...
// (written by canvas2d_rec_text_blocks) pre-populate the canvas's text cache so
...
// decoded RGBA8 the canvas has adopted (canvas2d_canvas_own_image -- patterns
...
struct replay_image {
    ...
    int w, h;
    enum canvas2d_color_type ct;  // the block's colour type, read by name...
    enum canvas2d_alpha_type at;  // ...and its alpha type, likewise
    enum canvas2d_color_space cs; // ...and its colour-space tag, likewise
    bool mips;    // an `image_mips` line ran: draws carry mip-chain semantics
};

struct replay_blocks {
    int map[CANVAS2D_FONT_INTERN_N];    // file font id -> interned cache id (-1 =
                                    // interning degraded; references still parse)
    bool seen[CANVAS2D_FONT_INTERN_N];  // file font id declared by a `font` line
    struct canvas2d_shaped *__single s;        // shape block under construction (owned
    ...
    enum canvas2d_color_type bm_ct;  // the pending image block's colour type
    enum canvas2d_alpha_type bm_at;  // ...and its alpha type
    enum canvas2d_color_space bm_cs; // ...and its colour-space tag
    long bm_gid;
    int bm_w, bm_h;
    float bm_ink[4];  // capture-px ink box x0 y0 x1 y1
    struct replay_image img[CANVAS2D_REC_IMAGES_MAX];  // declared `image` blocks
    ...
    // still expected.
    struct canvas2d_path2d *__single paths[CANVAS2D_REC_PATHS_MAX];
    struct canvas2d_path2d *__single pend_path;
    int pend_id;
...
// arrived, and the key copy).  canvas2d_shaped_free handles a partially-built
...
static void paths_drop(struct replay_blocks *__single b) {
    canvas2d_path2d_free(b->pend_path);
    b->pend_path = NULL;
    b->pend_id = 0;
    b->pend_cmds = 0;
    b->pend_left = 0;
    for (int i = 0; i < CANVAS2D_REC_PATHS_MAX; i++) {
        canvas2d_path2d_free(b->paths[i]);
        b->paths[i] = NULL;
    }
}
...
static void blocks_finish_shaping(struct canvas2d_context *__single cv,
                                struct replay_blocks *__single b) {
    canvas2d_text_cache_put_shaping(canvas2d_canvas_text_cache(cv), b->fam, b->fam_len,
                              b->size_px, b->rtl,
    ...
    // by the set_letter_spacing/set_word_spacing ops, recorded alongside the
    // block exactly as set_font_size is, so this block sets no spacing of its
    // own.
    blocks_drop(b);
}
...
static bool replay_font(struct canvas2d_context *__single cv, struct replay_blocks *__single b,
                        char const *__counted_by(le) data, size_t le, size_t j) {
    long file_id = 0;
    float vm[2];
    if (!read_uint(data, le, &j, CANVAS2D_FONT_INTERN_N - 1, &file_id) ||
        b->seen[file_id]) {
        return false;
    }
    ...
    b->seen[file_id] = true;
    int fid = canvas2d_text_cache_intern(canvas2d_canvas_text_cache(cv), data + j,
                                     (int)(le - j), (int)weight, style != 0);
    b->map[file_id] = fid;
    if (fid >= 0) {
        canvas2d_text_cache_set_vmetrics(canvas2d_canvas_text_cache(cv), fid,
                                     vm[0], vm[1]);
    }
    return true;
}
...
// numbers, upem >= 0.
static bool replay_glyph(struct canvas2d_context *__single cv, struct replay_blocks *__single b,
                         char const *__counted_by(le) data, size_t le, size_t j) {
    ...
    enum canvas2d_glyph_verb *verbs = NULL;
    canvas2d_vec2 *pts = NULL;
    if (nv > 0) {
        ...
        while (iv < nv && !at_eol(data, le, j)) {
            ...
            enum canvas2d_glyph_verb v = CANVAS2D_GLYPH_CLOSE;
            int k = 0;
            switch (data[ts]) {
                case 'm': v = CANVAS2D_GLYPH_MOVE;  k = 1; break;
                case 'l': v = CANVAS2D_GLYPH_LINE;  k = 1; break;
                case 'q': v = CANVAS2D_GLYPH_QUAD;  k = 2; break;
                case 'c': v = CANVAS2D_GLYPH_CUBIC; k = 3; break;
            ...
            for (int p = 0; p < k && ip < np; p++) {
                ...
                pts[ip++] = (canvas2d_vec2){ xy[0], xy[1] };
            }
        }
    }
    canvas2d_text_cache_put_glyph(canvas2d_canvas_text_cache(cv), b->map[file_id],
...
// CANVAS2D_CAPTURE_EM px to the em, deflated (canvas2d_zlib) to exactly <zlen> bytes
...
// captures empty ink), zlen in [1, canvas2d_zlib_bound(w*h*4)] (no stream our
...
static bool replay_bitmap(struct replay_blocks *__single b,
    ...
    if (!read_uint(data, le, &j, canvas2d_zlib_bound((int)total), &zlen) ||
        zlen < 1) {
...
// declare once), dims >= 1 with w*h*4 <= CANVAS2D_REC_IMAGE_BYTES_MAX (bounding
// the decoded allocation before either buffer exists), zlen in
// [1, canvas2d_zlib_bound(w*h*4)] and nlines in [1, ceil(zlen / 3)].
static bool replay_image(struct replay_blocks *__single b,
                         char const *__counted_by(le) data, size_t le, size_t j) {
    long id = 0, w = 0, h = 0, zlen = 0, nlines = 0;
    if (!read_uint(data, le, &j, CANVAS2D_REC_IMAGES_MAX - 1, &id) || b->img[id].px) {
        return false;
    }
    // The format axes, by name, like every other enum in the format.
    size_t ts, tl;
    enum canvas2d_color_type ct;
    if (!read_token(data, le, &j, &ts, &tl)) return false;
    if (tok_eq(data, le, ts, tl, "unorm8"))   ct = CANVAS2D_COLOR_UNORM8;
    else if (tok_eq(data, le, ts, tl, "f16")) ct = CANVAS2D_COLOR_F16;
    else return false;
    enum canvas2d_alpha_type at;
    if (!read_token(data, le, &j, &ts, &tl)) return false;
    if (tok_eq(data, le, ts, tl, "unpremul"))    at = CANVAS2D_ALPHA_UNPREMUL;
    else if (tok_eq(data, le, ts, tl, "premul")) at = CANVAS2D_ALPHA_PREMUL;
    else return false;
    long const bpp = ct == CANVAS2D_COLOR_F16 ? 8 : 4;
    long const dcap = CANVAS2D_REC_IMAGE_BYTES_MAX / bpp;  // a 1-px-tall image's max w
    if (!read_uint(data, le, &j, dcap, &w) || w < 1 ||
        !read_uint(data, le, &j, dcap, &h) || h < 1 ||
        w * h * bpp > CANVAS2D_REC_IMAGE_BYTES_MAX) {  // both <= 2^24: no overflow
        return false;
    }
    ...
    // The colour-space tag is the REQUIRED trailing token (the sampler converts
    // the resolved sample to the working space on deposit): sRGB, linear-sRGB,
    // and Oklab are equal peers, so every block carries its space by name -- a
    // missing token is a malformed file, not an sRGB default.  read_cs accepts
    // only the three colour-space names and requires the line to end after.
    enum canvas2d_color_space cs = CANVAS2D_CS_SRGB;
    if (!read_cs(data, le, &j, &cs)) {
        return false;
    }
    uint8_t *zs = malloc((size_t)zlen);
    if (!zs) {
...
// public canvas2d_path2d_* builders into a parser-owned object that
...
static bool replay_path(struct replay_blocks *__single b,
                        char const *__counted_by(le) data, size_t le, size_t j) {
    long id = 0, n = 0;
    if (!read_uint(data, le, &j, CANVAS2D_REC_PATHS_MAX - 1, &id) || b->paths[id]) {
        return false;
    }
...
// inflate (canvas2d_zlib_inflate, itself strict: header, Huffman structure,
...
// best-effort posture).
static bool replay_bits(struct canvas2d_context *__single cv, struct replay_blocks *__single b,
                        char const *__counted_by(le) data, size_t le, size_t j) {
    if (b->bm_lines <= 0) {
    ...
    if (b->bm_lines == 0) {
        ...
        if (canvas2d_zlib_inflate(px, b->bm_total, b->bm, b->bm_zlen) !=
            b->bm_total) {
            free(px);
        ...
        if (b->bm_img >= 0) {
            ...
            if (!canvas2d_canvas_own_image(cv, px, b->bm_total)) {
                free(px);
                return false;  // OOM while rebuilding: stop replay
            }
            ...
            canvas2d_text_cache_put_capture(canvas2d_canvas_text_cache(cv), b->bm_fid,
...
// shaping-toggle enums (canvas2d_font_kerning 0-2 / canvas2d_text_rendering 0-3,
// always present even at the AUTO default), variant-caps/stretch the small-cap /
// width enums (canvas2d_font_variant_caps 0-2 / canvas2d_font_stretch 0-8, always
...
static bool replay_shaping(struct canvas2d_context *__single cv, struct replay_blocks *__single b,
                         char const *__counted_by(le) data, size_t le, size_t j) {
    ...
    // The shaping toggles: kerning in canvas2d_font_kerning's [0, 2] and rendering
    // in canvas2d_text_rendering's [0, 3], so an out-of-range enum is a malformed
    // file (the live setter would have ignored it).
    if (!read_uint(data, le, &j, 2, &kerning) ||
        !read_uint(data, le, &j, 3, &rendering)) {
        return false;
    }
    // The small-cap / width toggles: variant-caps in canvas2d_font_variant_caps's
    // [0, 2] and stretch in canvas2d_font_stretch's [0, 8], strict like the others.
    ...
    struct canvas2d_shaped *s = calloc(1, sizeof *s);
    if (!s) {
        return false;
    }
    s->size_px = size;
    ...
    s->utf16s = (int)t16;
    if (nruns > 0) {
        struct canvas2d_glyph_run *runs = calloc((size_t)nruns, sizeof *runs);
        if (!runs) {
            free(s);
            return false;
        }
...
static bool replay_run(struct canvas2d_context *__single cv, struct replay_blocks *__single b,
                       char const *__counted_by(le) data, size_t le, size_t j) {
    if (!b->s) {
    ...
    struct canvas2d_glyph_run *run = &b->s->run[b->runs_done];
    run->glyph = g;
    run->xadv = adv;
    run->cluster = cl;
    run->count = (int)n;
...
static bool read_image_id(struct replay_blocks *__single b,
    ...
    long id = 0;
    if (!read_uint(data, le, jp, CANVAS2D_REC_IMAGES_MAX - 1, &id) ||
        !b->img[id].px) {
        return false;
    }
    *out = (int)id;
...
// valid -- the recorder's caps bound the block at CANVAS2D_REC_IMAGE_BYTES_MAX).
static bool replay_put_f16(struct canvas2d_context *__single cv, struct replay_image const *__single im,
                           int dx, int dy) {
    ...
    canvas2d_put_image_data_f16(cv, im->cs, px, len, im->w, im->h, dx, dy);
    free(px);
    return true;
}

static bool replay_put_dirty_f16(struct canvas2d_context *__single cv,
    ...
    memcpy(px, im->px, (size_t)im->len);
    canvas2d_put_image_data_dirty_f16(cv, im->cs, px, len, im->w, im->h, dx, dy,
                                    dirty_x, dirty_y, dirty_w, dirty_h);
    free(px);
    return true;
}

static bool replay_line(struct canvas2d_context *__single cv, struct replay_blocks *__single blk,
    ...
    if (tok_eq(data, le, cs, cl, "working_space")) {
        ...
        enum canvas2d_color_space sp;
        if      (tok_eq(data, le, ts, tl, canvas2d_color_space_name[CANVAS2D_CS_SRGB])) {
            sp = CANVAS2D_CS_SRGB;
        } else if (tok_eq(data, le, ts, tl, canvas2d_color_space_name[CANVAS2D_CS_LINEAR_SRGB])) {
            sp = CANVAS2D_CS_LINEAR_SRGB;
        } else {
            return false;
        }
        blk->started = true;
        return canvas2d_canvas_set_working_space(cv, sp);
    }
    ...
    // --- no-arg ---
    if (tok_eq(data, le, cs, cl, "save"))                 { canvas2d_save(cv); }
    else if (tok_eq(data, le, cs, cl, "restore"))         { canvas2d_restore(cv); }
    else if (tok_eq(data, le, cs, cl, "reset_transform")) { canvas2d_reset_transform(cv); }
    else if (tok_eq(data, le, cs, cl, "begin_path"))      { canvas2d_begin_path(cv); }
    else if (tok_eq(data, le, cs, cl, "close_path"))      { canvas2d_close_path(cv); }
    else if (tok_eq(data, le, cs, cl, "stroke"))          { canvas2d_stroke(cv); }
    else if (tok_eq(data, le, cs, cl, "set_filter_none")) { canvas2d_set_filter_none(cv); }
        ...
        // keep their ids.
        canvas2d_reset(cv);
        for (int k = 0; k < CANVAS2D_FONT_INTERN_N; k++) {
            blk->map[k] = -1;
            blk->seen[k] = false;
        }
    }
    ...
    //     it resets, restarting the font-id space like `reset`) ---
        ...
        if (!canvas2d_resize(cv, (int)w, (int)h)) {
    ...
    // --- 1 float ---
    else if (tok_eq(data, le, cs, cl, "rotate"))                { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_rotate(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_global_alpha"))      { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_global_alpha(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_line_width"))        { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_line_width(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_miter_limit"))       { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_miter_limit(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_line_dash_offset"))  { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_line_dash_offset(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_font_size"))         { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_font_size(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_letter_spacing"))    { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_letter_spacing(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_word_spacing"))      { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_word_spacing(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_shadow_blur"))       { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_shadow_blur(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_shadow_offset_x"))   { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_shadow_offset_x(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "set_shadow_offset_y"))   { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_set_shadow_offset_y(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_blur"))       { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_blur(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_brightness")) { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_brightness(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_contrast"))   { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_contrast(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_grayscale"))  { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_grayscale(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_hue_rotate")) { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_hue_rotate(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_invert"))     { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_invert(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_opacity"))    { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_opacity(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_saturate"))   { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_saturate(cv, f[0]); }
    else if (tok_eq(data, le, cs, cl, "add_filter_sepia"))      { if (!read_floats(data, le, &j, f, 1)) return false; canvas2d_add_filter_sepia(cv, f[0]); }

    // --- 2 float ---
    else if (tok_eq(data, le, cs, cl, "translate")) { if (!read_floats(data, le, &j, f, 2)) return false; canvas2d_translate(cv, f[0], f[1]); }
    else if (tok_eq(data, le, cs, cl, "scale"))     { if (!read_floats(data, le, &j, f, 2)) return false; canvas2d_scale(cv, f[0], f[1]); }
    else if (tok_eq(data, le, cs, cl, "move_to"))   { if (!read_floats(data, le, &j, f, 2)) return false; canvas2d_move_to(cv, f[0], f[1]); }
    else if (tok_eq(data, le, cs, cl, "line_to"))   { if (!read_floats(data, le, &j, f, 2)) return false; canvas2d_line_to(cv, f[0], f[1]); }
    // --- interp pair + 3 float ---
    else if (tok_eq(data, le, cs, cl, "set_fill_conic_gradient"))   { enum canvas2d_color_space sp; enum canvas2d_alpha_type al; if (!read_interp(data, le, &j, &sp, &al) || !read_floats(data, le, &j, f, 3)) return false; canvas2d_set_fill_conic_gradient(cv, sp, al, f[0], f[1], f[2]); }
    else if (tok_eq(data, le, cs, cl, "set_stroke_conic_gradient")) { enum canvas2d_color_space sp; enum canvas2d_alpha_type al; if (!read_interp(data, le, &j, &sp, &al) || !read_floats(data, le, &j, f, 3)) return false; canvas2d_set_stroke_conic_gradient(cv, sp, al, f[0], f[1], f[2]); }

    // --- 4 float ---
    else if (tok_eq(data, le, cs, cl, "rect"))                       { if (!read_floats(data, le, &j, f, 4)) return false; canvas2d_rect(cv, f[0], f[1], f[2], f[3]); }
    else if (tok_eq(data, le, cs, cl, "clear_rect"))                 { if (!read_floats(data, le, &j, f, 4)) return false; canvas2d_clear_rect(cv, f[0], f[1], f[2], f[3]); }
    else if (tok_eq(data, le, cs, cl, "fill_rect"))                  { if (!read_floats(data, le, &j, f, 4)) return false; canvas2d_fill_rect(cv, f[0], f[1], f[2], f[3]); }
    else if (tok_eq(data, le, cs, cl, "stroke_rect"))                { if (!read_floats(data, le, &j, f, 4)) return false; canvas2d_stroke_rect(cv, f[0], f[1], f[2], f[3]); }
    else if (tok_eq(data, le, cs, cl, "set_fill_rgba"))              { enum canvas2d_color_space sp; if (!read_floats(data, le, &j, f, 4) || !read_cs(data, le, &j, &sp)) return false; canvas2d_set_fill_rgba(cv, sp, f[0], f[1], f[2], f[3]); return true; }
    else if (tok_eq(data, le, cs, cl, "set_stroke_rgba"))            { enum canvas2d_color_space sp; if (!read_floats(data, le, &j, f, 4) || !read_cs(data, le, &j, &sp)) return false; canvas2d_set_stroke_rgba(cv, sp, f[0], f[1], f[2], f[3]); return true; }
    else if (tok_eq(data, le, cs, cl, "set_shadow_color_rgba"))      { enum canvas2d_color_space sp; if (!read_floats(data, le, &j, f, 4) || !read_cs(data, le, &j, &sp)) return false; canvas2d_set_shadow_color_rgba(cv, sp, f[0], f[1], f[2], f[3]); return true; }
    else if (tok_eq(data, le, cs, cl, "quadratic_curve_to"))         { if (!read_floats(data, le, &j, f, 4)) return false; canvas2d_quadratic_curve_to(cv, f[0], f[1], f[2], f[3]); }
    else if (tok_eq(data, le, cs, cl, "set_fill_linear_gradient"))   { enum canvas2d_color_space sp; enum canvas2d_alpha_type al; if (!read_interp(data, le, &j, &sp, &al) || !read_floats(data, le, &j, f, 4)) return false; canvas2d_set_fill_linear_gradient(cv, sp, al, f[0], f[1], f[2], f[3]); }
    else if (tok_eq(data, le, cs, cl, "set_stroke_linear_gradient")) { enum canvas2d_color_space sp; enum canvas2d_alpha_type al; if (!read_interp(data, le, &j, &sp, &al) || !read_floats(data, le, &j, f, 4)) return false; canvas2d_set_stroke_linear_gradient(cv, sp, al, f[0], f[1], f[2], f[3]); }

    // --- 5 float ---
    else if (tok_eq(data, le, cs, cl, "round_rect"))            { if (!read_floats(data, le, &j, f, 5)) return false; canvas2d_round_rect(cv, f[0], f[1], f[2], f[3], f[4]); }
    else if (tok_eq(data, le, cs, cl, "arc_to"))                { if (!read_floats(data, le, &j, f, 5)) return false; canvas2d_arc_to(cv, f[0], f[1], f[2], f[3], f[4]); }
    else if (tok_eq(data, le, cs, cl, "add_fill_color_stop"))   { enum canvas2d_color_space sp; if (!read_floats(data, le, &j, f, 5) || !read_cs(data, le, &j, &sp)) return false; canvas2d_add_fill_color_stop(cv, sp, f[0], f[1], f[2], f[3], f[4]); return true; }
    else if (tok_eq(data, le, cs, cl, "add_stroke_color_stop")) { enum canvas2d_color_space sp; if (!read_floats(data, le, &j, f, 5) || !read_cs(data, le, &j, &sp)) return false; canvas2d_add_stroke_color_stop(cv, sp, f[0], f[1], f[2], f[3], f[4]); return true; }

    // --- 9 float (the full 3x3 CTM; affine lines carry a trailing 0 0 1) ---
    else if (tok_eq(data, le, cs, cl, "transform"))                  { if (!read_floats(data, le, &j, f, 9)) return false; canvas2d_transform_3x3(cv, f[0], f[1], f[2], f[3], f[4], f[5], f[6], f[7], f[8]); }
    else if (tok_eq(data, le, cs, cl, "set_transform"))              { if (!read_floats(data, le, &j, f, 9)) return false; canvas2d_set_transform_3x3(cv, f[0], f[1], f[2], f[3], f[4], f[5], f[6], f[7], f[8]); }

    // --- 6 float ---
    else if (tok_eq(data, le, cs, cl, "bezier_curve_to"))            { if (!read_floats(data, le, &j, f, 6)) return false; canvas2d_bezier_curve_to(cv, f[0], f[1], f[2], f[3], f[4], f[5]); }
    else if (tok_eq(data, le, cs, cl, "set_fill_radial_gradient"))   { enum canvas2d_color_space sp; enum canvas2d_alpha_type al; if (!read_interp(data, le, &j, &sp, &al) || !read_floats(data, le, &j, f, 6)) return false; canvas2d_set_fill_radial_gradient(cv, sp, al, f[0], f[1], f[2], f[3], f[4], f[5]); }
    else if (tok_eq(data, le, cs, cl, "set_stroke_radial_gradient")) { enum canvas2d_color_space sp; enum canvas2d_alpha_type al; if (!read_interp(data, le, &j, &sp, &al) || !read_floats(data, le, &j, f, 6)) return false; canvas2d_set_stroke_radial_gradient(cv, sp, al, f[0], f[1], f[2], f[3], f[4], f[5]); }

    // --- 7 float ---
    else if (tok_eq(data, le, cs, cl, "add_filter_drop_shadow")) {
        enum canvas2d_color_space sp;
        if (!read_floats(data, le, &j, f, 7) || !read_cs(data, le, &j, &sp)) return false;
        canvas2d_add_filter_drop_shadow(cv, sp, f[0], f[1], f[2], f[3], f[4], f[5], f[6]);
        return true;
    }
    ...
    // --- 12 float ---
        ...
        canvas2d_round_rect_radii(cv, rr[0], rr[1], rr[2], rr[3], rr[4], rr[5],
    ...
    // --- enums by name ---
        ...
        if (tok_eq(data, le, ts, tl, "nonzero"))      rule = CANVAS2D_NONZERO;
        else if (tok_eq(data, le, ts, tl, "evenodd")) rule = CANVAS2D_EVENODD;
        else return false;
        if (fill) canvas2d_fill(cv, rule);
        else      canvas2d_clip(cv, rule);
    }
    else if (tok_eq(data, le, cs, cl, "set_line_join")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "miter"))      canvas2d_set_line_join(cv, CANVAS2D_JOIN_MITER);
        else if (tok_eq(data, le, ts, tl, "round")) canvas2d_set_line_join(cv, CANVAS2D_JOIN_ROUND);
        else if (tok_eq(data, le, ts, tl, "bevel")) canvas2d_set_line_join(cv, CANVAS2D_JOIN_BEVEL);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_line_cap")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "butt"))        canvas2d_set_line_cap(cv, CANVAS2D_CAP_BUTT);
        else if (tok_eq(data, le, ts, tl, "round"))  canvas2d_set_line_cap(cv, CANVAS2D_CAP_ROUND);
        else if (tok_eq(data, le, ts, tl, "square")) canvas2d_set_line_cap(cv, CANVAS2D_CAP_SQUARE);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_text_align")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "start"))       canvas2d_set_text_align(cv, CANVAS2D_ALIGN_START);
        else if (tok_eq(data, le, ts, tl, "end"))    canvas2d_set_text_align(cv, CANVAS2D_ALIGN_END);
        else if (tok_eq(data, le, ts, tl, "left"))   canvas2d_set_text_align(cv, CANVAS2D_ALIGN_LEFT);
        else if (tok_eq(data, le, ts, tl, "right"))  canvas2d_set_text_align(cv, CANVAS2D_ALIGN_RIGHT);
        else if (tok_eq(data, le, ts, tl, "center")) canvas2d_set_text_align(cv, CANVAS2D_ALIGN_CENTER);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_text_baseline")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "alphabetic"))       canvas2d_set_text_baseline(cv, CANVAS2D_BASELINE_ALPHABETIC);
        else if (tok_eq(data, le, ts, tl, "top"))         canvas2d_set_text_baseline(cv, CANVAS2D_BASELINE_TOP);
        else if (tok_eq(data, le, ts, tl, "hanging"))     canvas2d_set_text_baseline(cv, CANVAS2D_BASELINE_HANGING);
        else if (tok_eq(data, le, ts, tl, "middle"))      canvas2d_set_text_baseline(cv, CANVAS2D_BASELINE_MIDDLE);
        else if (tok_eq(data, le, ts, tl, "ideographic")) canvas2d_set_text_baseline(cv, CANVAS2D_BASELINE_IDEOGRAPHIC);
        else if (tok_eq(data, le, ts, tl, "bottom"))      canvas2d_set_text_baseline(cv, CANVAS2D_BASELINE_BOTTOM);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_direction")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "ltr"))      canvas2d_set_direction(cv, CANVAS2D_DIRECTION_LTR);
        else if (tok_eq(data, le, ts, tl, "rtl")) canvas2d_set_direction(cv, CANVAS2D_DIRECTION_RTL);
        else return false;
    }
        ...
        canvas2d_set_font_family_n(cv, data + j, (int)fl);
        ...
        canvas2d_set_font_weight(cv, (int)w);
    }
    else if (tok_eq(data, le, cs, cl, "set_font_style")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "normal"))      canvas2d_set_font_style(cv, CANVAS2D_FONT_STYLE_NORMAL);
        else if (tok_eq(data, le, ts, tl, "italic")) canvas2d_set_font_style(cv, CANVAS2D_FONT_STYLE_ITALIC);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_font_kerning")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "auto"))        canvas2d_set_font_kerning(cv, CANVAS2D_FONT_KERNING_AUTO);
        else if (tok_eq(data, le, ts, tl, "normal")) canvas2d_set_font_kerning(cv, CANVAS2D_FONT_KERNING_NORMAL);
        else if (tok_eq(data, le, ts, tl, "none"))   canvas2d_set_font_kerning(cv, CANVAS2D_FONT_KERNING_NONE);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_text_rendering")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "auto"))                     canvas2d_set_text_rendering(cv, CANVAS2D_TEXT_RENDERING_AUTO);
        else if (tok_eq(data, le, ts, tl, "optimizeSpeed"))       canvas2d_set_text_rendering(cv, CANVAS2D_TEXT_RENDERING_OPTIMIZE_SPEED);
        else if (tok_eq(data, le, ts, tl, "optimizeLegibility"))  canvas2d_set_text_rendering(cv, CANVAS2D_TEXT_RENDERING_OPTIMIZE_LEGIBILITY);
        else if (tok_eq(data, le, ts, tl, "geometricPrecision"))  canvas2d_set_text_rendering(cv, CANVAS2D_TEXT_RENDERING_GEOMETRIC_PRECISION);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_font_variant_caps")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "normal"))               canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_NORMAL);
        else if (tok_eq(data, le, ts, tl, "small-caps"))      canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_SMALL_CAPS);
        else if (tok_eq(data, le, ts, tl, "all-small-caps"))  canvas2d_set_font_variant_caps(cv, CANVAS2D_FONT_VARIANT_CAPS_ALL_SMALL_CAPS);
        else return false;
    }
    else if (tok_eq(data, le, cs, cl, "set_font_stretch")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "ultra-condensed"))      canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_ULTRA_CONDENSED);
        else if (tok_eq(data, le, ts, tl, "extra-condensed")) canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_EXTRA_CONDENSED);
        else if (tok_eq(data, le, ts, tl, "condensed"))       canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_CONDENSED);
        else if (tok_eq(data, le, ts, tl, "semi-condensed"))  canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_SEMI_CONDENSED);
        else if (tok_eq(data, le, ts, tl, "normal"))          canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_NORMAL);
        else if (tok_eq(data, le, ts, tl, "semi-expanded"))   canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_SEMI_EXPANDED);
        else if (tok_eq(data, le, ts, tl, "expanded"))        canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_EXPANDED);
        else if (tok_eq(data, le, ts, tl, "extra-expanded"))  canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_EXTRA_EXPANDED);
        else if (tok_eq(data, le, ts, tl, "ultra-expanded"))  canvas2d_set_font_stretch(cv, CANVAS2D_FONT_STRETCH_ULTRA_EXPANDED);
        else return false;
    }
        ...
        canvas2d_set_lang_n(cv, data + j, (int)ll);
        return true;  // the tag consumed the rest of the line
    }
    else if (tok_eq(data, le, cs, cl, "set_image_smoothing_quality")) {
        size_t ts, tl;
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        if (tok_eq(data, le, ts, tl, "low"))         canvas2d_set_image_smoothing_quality(cv, CANVAS2D_SMOOTHING_LOW);
        else if (tok_eq(data, le, ts, tl, "medium")) canvas2d_set_image_smoothing_quality(cv, CANVAS2D_SMOOTHING_MEDIUM);
        else if (tok_eq(data, le, ts, tl, "high"))   canvas2d_set_image_smoothing_quality(cv, CANVAS2D_SMOOTHING_HIGH);
        else return false;
    }
        ...
        if (!read_token(data, le, &j, &ts, &tl)) return false;
        int op = -1;
        for (int k = 0; k < (int)(sizeof canvas2d_composite_name / sizeof canvas2d_composite_name[0]); k++) {
            if (tok_eq(data, le, ts, tl, canvas2d_composite_name[k])) { op = k; break; }
        }
        if (op < 0) return false;
        canvas2d_set_global_composite_operation(cv, (enum canvas2d_composite_op)op);
    }
    ...
    // --- path blocks + the Path2D ops that reference them by id ---
        ...
        long id = 0;
        size_t ts, tl;
        if (!read_uint(data, le, &j, CANVAS2D_REC_PATHS_MAX - 1, &id) ||
            !blk->paths[id] || !read_token(data, le, &j, &ts, &tl)) return false;
        enum canvas2d_fill_rule rule;
        if (tok_eq(data, le, ts, tl, "nonzero"))      rule = CANVAS2D_NONZERO;
        else if (tok_eq(data, le, ts, tl, "evenodd")) rule = CANVAS2D_EVENODD;
        else return false;
        if (fill) canvas2d_fill_path(cv, blk->paths[id], rule);
        else      canvas2d_clip_path(cv, blk->paths[id], rule);
    }
    else if (tok_eq(data, le, cs, cl, "stroke_path")) {
        long id = 0;
        if (!read_uint(data, le, &j, CANVAS2D_REC_PATHS_MAX - 1, &id) ||
            !blk->paths[id]) return false;
        canvas2d_stroke_path(cv, blk->paths[id]);
    }
        ...
        canvas2d_canvas_draw_block(cv, im.px, im.len, im.w, im.h, im.ct,
        ...
        // block's colour-space tag, so replay re-applies whatever the block
        // declared -- every space round-trips.  An alpha type other than
        // UNPREMUL, or a colour type that is neither flavour, is a malformed
        // file.
        struct replay_image const im = blk->img[id];
        if (im.at != CANVAS2D_ALPHA_UNPREMUL) return false;
        if (im.ct == CANVAS2D_COLOR_UNORM8) {
            canvas2d_put_image_data(cv, im.cs, im.px, im.len, im.w, im.h,
                                  (int)v[0], (int)v[1]);
        } else if (im.ct == CANVAS2D_COLOR_F16) {
        ...
        // tag, exactly as for put_image_data above.
        struct replay_image const im = blk->img[id];
        if (im.at != CANVAS2D_ALPHA_UNPREMUL) return false;
        if (im.ct == CANVAS2D_COLOR_UNORM8) {
            canvas2d_put_image_data_dirty(cv, im.cs, im.px, im.len, im.w, im.h,
        ...
        if (blk->img[id].ct != CANVAS2D_COLOR_UNORM8 ||
            blk->img[id].at != CANVAS2D_ALPHA_UNPREMUL) return false;
        int rep = -1;
        for (int k = 0; k < (int)(sizeof canvas2d_repeat_name / sizeof canvas2d_repeat_name[0]); k++) {
            if (tok_eq(data, le, ts, tl, canvas2d_repeat_name[k])) { rep = k; break; }
        }
        ...
        if (fill) {
            canvas2d_set_fill_pattern(cv, blk->img[id].cs, blk->img[id].px,
                                    blk->img[id].w, blk->img[id].h,
                                    (enum canvas2d_pattern_repeat)rep);
        } else {
            canvas2d_set_stroke_pattern(cv, blk->img[id].cs, blk->img[id].px,
                                      blk->img[id].w, blk->img[id].h,
                                      (enum canvas2d_pattern_repeat)rep);
        }
    }
    ...
    // --- text tail (rest of line, UTF-8) ---
        ...
        if (fill) canvas2d_fill_text_n(cv, data + j, n, f[0], f[1]);
        else      canvas2d_stroke_text_n(cv, data + j, n, f[0], f[1]);
        ...
        if (fill) canvas2d_fill_text_max_n(cv, data + j, n, f[0], f[1], f[2]);
        else      canvas2d_stroke_text_max_n(cv, data + j, n, f[0], f[1], f[2]);
        return true;  // text consumed the rest of the line
    }
...
bool canvas2d_replay_from(struct canvas2d_context *__single cv, char const *__null_terminated path) {
    ...
    (void)fclose(f);
    bool const ok = canvas2d_replay_text(cv, buf, got);
    free(buf);
    return ok;
}
