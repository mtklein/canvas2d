...
// with a linear->sRGB encode (canvas2d_linear_to_srgb) inserted between the divide
...
static canvas2d_px8 unpremul_encode_to_unorm8(canvas2d_px8 p) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (a > 0.0f) {  // a transparent lane stays all-zero (the planar mask's twin)
            ...
            r  = canvas2d_clamp01(canvas2d_linear_to_srgb((float)p.r[i] * inv));
            g  = canvas2d_clamp01(canvas2d_linear_to_srgb((float)p.g[i] * inv));
            b  = canvas2d_clamp01(canvas2d_linear_to_srgb((float)p.b[i] * inv));
            ca = canvas2d_clamp01(a);
        }
...
#define CANVAS2D_OKLAB_AB_BIAS 0.5f
...
static canvas2d_px8 unpremul_to_oklab_unorm8(canvas2d_px8 p, bool linear_canvas) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (a > 0.0f) {  // a transparent lane stays all-zero
            float const inv = 1.0f / a;
            canvas2d_rgb lin = { .r = (float)p.r[i] * inv,
                             .g = (float)p.g[i] * inv,
                             .b = (float)p.b[i] * inv };
            if (!linear_canvas) {  // stored encoded sRGB -> linear first
                lin = canvas2d_rgb_srgb_to_linear(lin);
            }
            canvas2d_oklab const lab = canvas2d_linear_srgb_to_oklab(lin);
            qL = canvas2d_clamp01(lab.L);
            qa = canvas2d_clamp01(lab.a + CANVAS2D_OKLAB_AB_BIAS);
            qb = canvas2d_clamp01(lab.b + CANVAS2D_OKLAB_AB_BIAS);
            ca = canvas2d_clamp01(a);
        }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (a > 0.0f) {  // a transparent lane stays all-zero
            ...
            if (!linear_canvas) {  // stored encoded sRGB -> linear
                r = canvas2d_srgb_to_linear(r);
                g = canvas2d_srgb_to_linear(g);
                b = canvas2d_srgb_to_linear(b);
            }
            r  = canvas2d_clamp01(r);
            g  = canvas2d_clamp01(g);
            b  = canvas2d_clamp01(b);
            ca = canvas2d_clamp01(a);
        }
        o.r[i] = (_Float16)(r  * 255.0f + 0.5f);
        o.g[i] = (_Float16)(g  * 255.0f + 0.5f);
...
// Per-canvas, per-output-space readback.  CANVAS2D_CS_SRGB off an sRGB canvas is a
...
static canvas2d_px8 read_unorm8(struct canvas2d_context *__single cv,
    ...
    // CANVAS2D_CS_SRGB (and any out-of-range value): never a decode/encode round
    // trip.
...
// re-interleaving at the RGBA8 seam (canvas2d_planar.h); the n%8 tail runs the same
// slab gathered.  `space` names the space the OUTPUT bytes are encoded in:
//   CANVAS2D_CS_SRGB        -- encoded sRGB (an sRGB canvas is the SIMD bypass, a
//                            linear canvas encodes linear->sRGB).  NOT a linear
//                            round trip.
//   CANVAS2D_CS_LINEAR_SRGB -- linear-sRGB bytes (linear canvas: the stored values
//                            quantized directly; sRGB canvas: decode then quantize).
//   CANVAS2D_CS_OKLAB       -- Oklab (L,a,b) bytes (working->linear->Oklab).
// get_image_data reads back through this same entry point.  write_png does NOT
// route here: it goes through canvas2d_encode_png -> surface_to_pq16 (16-bit
...
void canvas2d_read_rgba(struct canvas2d_context *__single cv, enum canvas2d_color_space space,
    ...
    int i = 0;
    for (; i + 8 <= n; i += 8) {
        canvas2d_px8_store_rgba8(out + i * 4,
                             read_unorm8(cv, space, canvas2d_px8_load(cv->target + i)));
    }
    if (i < n) {
        int const k = n - i;
        canvas2d_px8_store_rgba8_k(out + i * 4, k,
                               read_unorm8(cv, space, canvas2d_px8_load_k(cv->target + i, k)));
    }
}
...
static canvas2d_px8 unpremul_to_srgb_f16(canvas2d_px8 p, bool linear_canvas) {
    ...
    for (int i = 0; i < 8; i++) {  // linear canvas: encode the linear colour to sRGB
        u.r[i] = (_Float16)canvas2d_linear_to_srgb((float)u.r[i]);
        u.g[i] = (_Float16)canvas2d_linear_to_srgb((float)u.g[i]);
        u.b[i] = (_Float16)canvas2d_linear_to_srgb((float)u.b[i]);
    }
    return u;
}
...
static canvas2d_px8 unpremul_to_linear_f16(canvas2d_px8 p, bool linear_canvas) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (a > 0.0f) {  // a transparent lane stays all-zero
            ...
            o.g[i] = (_Float16)g;
            o.b[i] = (_Float16)b;
            o.a[i] = (_Float16)canvas2d_clamp01(a);
        }
    }
    return o;
}
...
static canvas2d_px8 read_f16(struct canvas2d_context *__single cv,
                         enum canvas2d_color_space space, canvas2d_px8 p) {
    bool const lin = cv->space == CANVAS2D_CS_LINEAR_SRGB;
    if (space == CANVAS2D_CS_LINEAR_SRGB) {
    ...
    return unpremul_to_srgb_f16(p, lin);  // CANVAS2D_CS_SRGB (and any out-of-range)
}

// canvas2d_read_rgba's f16 twin: read the whole canvas back as straight
// (unpremultiplied) _Float16 RGBA into `out` (canvas2d_unpremul-shaped, w*h pixels)
// in the requested OUTPUT space, extended range preserved.  Eight pixels per
// slab with st4 re-interleave at the unpremul seam (canvas2d_planar.h); the n%8 tail
// runs the same slab gathered.  Internal: get_image_data_f16 reads through here.
static void canvas2d_read_rgba_f16(struct canvas2d_context *__single cv,
                                 enum canvas2d_color_space space,
                                 canvas2d_unpremul *__counted_by(npx) out, int npx) {
    ...
    int i = 0;
    for (; i + 8 <= n; i += 8) {
        canvas2d_px8_store_unpremul(out + i, read_f16(cv, space, canvas2d_px8_load(cv->target + i)));
    }
    if (i < n) {
        int const k = n - i;
        canvas2d_px8_store_unpremul_k(out + i, k,
                                  read_f16(cv, space, canvas2d_px8_load_k(cv->target + i, k)));
    }
}
...
void canvas2d_get_image_data(struct canvas2d_context *__single cv,
                           ...
                           uint8_t *__counted_by(len) out, int len) {
    if (!canvas2d_rgba8_dims_ok(w, h) || len < w * h * 4) {
        return;
    }
    ...
    canvas2d_read_rgba(cv, space, buf, clen);  // reads back in the requested space
    canvas2d_blit_rgba(out, w, h, 0, 0, buf, cv->width, cv->height, x, y, w, h);
    free(buf);
}
...
canvas2d_create_image_data(int sw, int sh, int *__single len) {
    ...
    // canvas2d_rgba8_dims_ok guarantees sw*sh*4 fits a positive int, so this is overflow-free.
    int const n = sw * sh * 4;
...
// canvas2d_get_image_data's f16 twin (rgba-float16 ImageData): read the w*h
...
void canvas2d_get_image_data_f16(struct canvas2d_context *__single cv,
    ...
    canvas2d_unpremul *__counted_by_or_null(cpx) buf = malloc((size_t)cpx * sizeof *buf);
    if (!buf) {
        return;
    }
    canvas2d_read_rgba_f16(cv, space, buf, cpx);  // reads back in the requested space
    // canvas2d_unpremul is four contiguous _Float16, so buf is a w*h*4-element
    // _Float16 image -- the layout canvas2d_blit_f16 copies, four components/pixel.
    canvas2d_blit_f16(out, w, h, 0, 0, (_Float16 const *)buf, cv->width, cv->height,
                  x, y, w, h);
    free(buf);
}

// canvas2d_create_image_data's f16 twin: a blank (transparent black) rgba-float16
...
// Scalar per lane (the f32 decode, canvas2d_color.c's deferral note); incoming
// putImageData bytes are in [0,1] so the decode stays in [0,1], no extended
// values to carry.  Reached only on cv->space == CANVAS2D_CS_LINEAR_SRGB.
static canvas2d_px8 px8_decode_rgb(canvas2d_px8 p) {
    for (int i = 0; i < 8; i++) {
        p.r[i] = (_Float16)canvas2d_srgb_to_linear((float)p.r[i]);
        p.g[i] = (_Float16)canvas2d_srgb_to_linear((float)p.g[i]);
        p.b[i] = (_Float16)canvas2d_srgb_to_linear((float)p.b[i]);
    }
    return p;
}
...
static canvas2d_px8 px8_in_linear(canvas2d_px8 p, bool linear_canvas) {
    if (linear_canvas) {
        return p;  // input already linear, working space linear: store as-is
    }
    for (int i = 0; i < 8; i++) {  // sRGB working canvas: encode linear->sRGB
        p.r[i] = (_Float16)canvas2d_linear_to_srgb((float)p.r[i]);
        p.g[i] = (_Float16)canvas2d_linear_to_srgb((float)p.g[i]);
        p.b[i] = (_Float16)canvas2d_linear_to_srgb((float)p.b[i]);
    }
    return p;
}

// Incoming bytes are an Oklab (L,a,b) triple in the CANVAS2D_OKLAB_AB_BIAS
// convention (above): un-bias a,b, Oklab->linear sRGB, then the linear handling.
static canvas2d_px8 px8_in_oklab(canvas2d_px8 p, bool linear_canvas) {
    for (int i = 0; i < 8; i++) {
        canvas2d_oklab const lab = { .L = (float)p.r[i],
                                 .a = (float)p.g[i] - CANVAS2D_OKLAB_AB_BIAS,
                                 .b = (float)p.b[i] - CANVAS2D_OKLAB_AB_BIAS };
        canvas2d_rgb lin = canvas2d_oklab_to_linear_srgb(lab);
        if (!linear_canvas) {  // sRGB working canvas: encode linear->sRGB
            lin = canvas2d_rgb_linear_to_srgb(lin);
        }
...
static canvas2d_px8 put_to_working(canvas2d_px8 p, enum canvas2d_color_space space,
                               bool linear_canvas) {
    if (space == CANVAS2D_CS_LINEAR_SRGB) {
    ...
    // CANVAS2D_CS_SRGB (and any out-of-range value): px8_decode_rgb on a linear
...
// arrive UNBIASED (the f16 path carries Oklab straight -- no CANVAS2D_OKLAB_AB_BIAS
...
static canvas2d_px8 px8_in_oklab_f16(canvas2d_px8 p, bool linear_canvas) {
    for (int i = 0; i < 8; i++) {
        canvas2d_oklab const lab = { .L = (float)p.r[i],
                                 .a = (float)p.g[i],
                                 .b = (float)p.b[i] };
...
static canvas2d_px8 put_to_working_f16(canvas2d_px8 p, enum canvas2d_color_space space,
                                   bool linear_canvas) {
    if (space == CANVAS2D_CS_OKLAB) {
...
static void put_image_sub(struct canvas2d_context *__single cv,
                          enum canvas2d_color_space space,
    ...
    if (rw <= 0 || rh <= 0 || !canvas2d_ensure_tile(cv, rw * rh)) {
        return;
    }
    ...
    // planes (canvas2d_planar.h), a true _Float16 divide scales each to [0,1],
    ...
    // encoded-sRGB input (CANVAS2D_CS_SRGB) put_to_working is px8_decode_rgb on a
    ...
    for (int py = 0; py < rh; py++) {
        ...
        for (; px + 8 <= rw; px += 8) {
            ...
            canvas2d_px8 p = canvas2d_px8_load_rgba8(data + si);
            p = (canvas2d_px8){ p.r / k255, p.g / k255, p.b / k255, p.a / k255 };
            p = put_to_working(p, space, lin);
            canvas2d_px8_store(cv->tile + py * rw + px, canvas2d_px8_premultiply(p));
        }
        if (px < rw) {
            int const k = rw - px;
            int const si = ((row0 + py) * w + (col0 + px)) * 4;
            canvas2d_px8 p = canvas2d_px8_load_rgba8_k(data + si, k);
            p = (canvas2d_px8){ p.r / k255, p.g / k255, p.b / k255, p.a / k255 };
            p = put_to_working(p, space, lin);
            canvas2d_px8_store_k(cv->tile + py * rw + px, k, canvas2d_px8_premultiply(p));
        }
    }
    // putImageData overwrites and ignores the clip: composite COPY with no clip.
    canvas2d_blend(cv, (int)cx0, (int)cy0, rw, rh, cv->tile, NULL, NULL, 0,
               CANVAS2D_OP_COPY);
}
...
// canvas2d_px8_load_unpremul, put_to_working (total -- works on extended values),
// then a NON-clamping premultiply (canvas2d_px8_premultiply_unclamped keeps the
// colour planes unbounded; only alpha clamps into [0,1]).  The COPY blend's
// output clamp is colour-unbounded on a linear canvas (canvas2d_px8_clamp_premul_lin)
...
// `data` is viewed as canvas2d_unpremul (four contiguous _Float16, the ImageData
// layout).
static void put_image_sub_f16(struct canvas2d_context *__single cv,
                              enum canvas2d_color_space space,
    ...
    int const row0 = (int)(cy0 - dy);
    bool const lin = cv->space == CANVAS2D_CS_LINEAR_SRGB;
    // The w*h*4-element _Float16 source viewed as w*h canvas2d_unpremul pixels; the
    ...
    canvas2d_unpremul const *__counted_by(npx) src = (canvas2d_unpremul const *)data;
    for (int py = 0; py < rh; py++) {
        int px = 0;
        for (; px + 8 <= rw; px += 8) {
            int const si = (row0 + py) * w + (col0 + px);  // pixel index, in [0, npx)
            canvas2d_px8 p = canvas2d_px8_load_unpremul(src + si);  // straight f16, NO /255
            p = put_to_working_f16(p, space, lin);
            canvas2d_px8_store(cv->tile + py * rw + px, canvas2d_px8_premultiply_unclamped(p));
        }
        if (px < rw) {
            int const k = rw - px;
            int const si = (row0 + py) * w + (col0 + px);
            canvas2d_px8 p = canvas2d_px8_load_unpremul_k(src + si, k);
            p = put_to_working_f16(p, space, lin);
            canvas2d_px8_store_k(cv->tile + py * rw + px, k, canvas2d_px8_premultiply_unclamped(p));
        }
    }
    // putImageData overwrites and ignores the clip: composite COPY with no clip.
    canvas2d_blend(cv, (int)cx0, (int)cy0, rw, rh, cv->tile, NULL, NULL, 0,
...
void canvas2d_put_image_data(struct canvas2d_context *__single cv,
    ...
    if (cv->rec) {
        ...
        // BLOCK's colour-space tag (canvas2d_rec_image), written for every space, so
        // every put_image_data round-trips through the tag (replay reads it back
        // off the block).
        int const id = canvas2d_rec_image(cv->rec, data, w * h * 4, w, h,
                                      CANVAS2D_COLOR_UNORM8, CANVAS2D_ALPHA_UNPREMUL,
                                      space);
        if (id >= 0) {
            canvas2d_rec_image_ints(cv->rec, "put_image_data", id,
                                (int[]){ dx, dy }, 2);
        }
    }
...
void canvas2d_put_image_data_dirty(struct canvas2d_context *__single cv,
    ...
    if (cv->rec) {
        ...
        int const id = canvas2d_rec_image(cv->rec, data, w * h * 4, w, h,
                                      ...
                                      space);
        if (id >= 0) {
            canvas2d_rec_image_ints(cv->rec, "put_image_data_dirty", id,
...
// canvas2d_put_image_data's f16 twin (rgba-float16 ImageData): the source is
...
// put -- the f16-ness rides the image block's colour type (CANVAS2D_COLOR_F16);
...
void canvas2d_put_image_data_f16(struct canvas2d_context *__single cv,
    ...
    if (cv->rec) {
        // The block carries w*h*4 _Float16 (== w*h*8 bytes) as CANVAS2D_COLOR_F16;
        ...
        int const id = canvas2d_rec_image(cv->rec, (uint8_t const *)data,
                                      w * h * 4 * (int)sizeof(_Float16), w, h,
                                      CANVAS2D_COLOR_F16, CANVAS2D_ALPHA_UNPREMUL,
                                      space);
        if (id >= 0) {
            canvas2d_rec_image_ints(cv->rec, "put_image_data", id,
                                (int[]){ dx, dy }, 2);
...
void canvas2d_put_image_data_dirty_f16(struct canvas2d_context *__single cv,
    ...
    // Normalise the dirty rect exactly as canvas2d_put_image_data_dirty does.
    int64_t dxx = dirty_x, dyy = dirty_y, dww = dirty_w, dhh = dirty_h;
    if (dww < 0) { dxx += dww; dww = -dww; }
    if (dhh < 0) { dyy += dhh; dhh = -dhh; }
    if (dxx < 0) { dww += dxx; dxx = 0; }
