...
// canvas owns it until canvas2d_free (implemented in canvas2d.c).  The canvas
...
// Reconfigure the canvas's immutable working space (canvas2d.h) from a replayed
// `working_space` line (implemented in canvas2d.c).  The space is normally fixed
...
// Draw one replayed image block (implemented in canvas2d.c, the replay-side
