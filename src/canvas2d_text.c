...
float canvas2d_shaped_width(struct canvas2d_shaped const *__single s) {
    if (!s) {
        return 0.0f;
    }
    float w = 0.0f;
    for (int r = 0; r < s->nruns; r++) {
        struct canvas2d_glyph_run const run = s->run[r];
...
void canvas2d_shaped_apply_spacing(struct canvas2d_shaped *__single s,
    ...
    for (int r = 0; r < s->nruns; r++) {
        struct canvas2d_glyph_run *__single run = &s->run[r];
        int i = 0;
        while (i < run->count) {
...
int canvas2d_shaped_index_at_x(struct canvas2d_shaped const *__single s, float x) {
    if (!s) {
        return -1;
    }
    float pen = 0.0f;
    for (int r = 0; r < s->nruns; r++) {
        struct canvas2d_glyph_run const run = s->run[r];  // glyphs are visual-order, so a single
    ...
    for (int r = 0; r < s->nruns; r++) {
        ...
        for (int i = 0; i < run.count; i++) {
            ...
            if (sel && !in) {
                ...
                if (n < max) {
                    out[n++] = (canvas2d_xspan){ start, pen };
                }
                in = false;
            }
            pen += run.xadv[i];
...
static int verb_pts(enum canvas2d_glyph_verb v) {
    switch (v) {
        case CANVAS2D_GLYPH_MOVE:
        case CANVAS2D_GLYPH_LINE:  return 1;
        case CANVAS2D_GLYPH_QUAD:  return 2;
        case CANVAS2D_GLYPH_CUBIC: return 3;
        case CANVAS2D_GLYPH_CLOSE: return 0;
    }
...
static void walk_curves(enum canvas2d_glyph_verb const *__counted_by(nv) verb, int nv,
                        canvas2d_vec2 const *__counted_by(np) pt, int np,
                        struct glyph_place const *__single g, float tol,
                        struct canvas2d_path *__single out) {
    int ip = 0;
    for (int iv = 0; iv < nv; iv++) {
        enum canvas2d_glyph_verb const v = verb[iv];
        ...
        switch (v) {
            case CANVAS2D_GLYPH_MOVE:
                canvas2d_path_move_to(out, place(g, pt[ip]));
                break;
            case CANVAS2D_GLYPH_LINE:
                canvas2d_path_line_to(out, place(g, pt[ip]));
                break;
            case CANVAS2D_GLYPH_QUAD:
                canvas2d_path_quad_to(out, place(g, pt[ip]), place(g, pt[ip + 1]), tol);
                break;
            case CANVAS2D_GLYPH_CUBIC:
                canvas2d_path_cubic_to(out, place(g, pt[ip]), place(g, pt[ip + 1]),
                                   place(g, pt[ip + 2]), tol);
                break;
            case CANVAS2D_GLYPH_CLOSE:
...
void canvas2d_text_cache_init(struct canvas2d_text_cache *__single c) {
    memset(c, 0, sizeof *c);  // empty slots: NULL pointers with zero counts
}

void canvas2d_text_cache_reset(struct canvas2d_text_cache *__single c) {
    for (int i = 0; i < CANVAS2D_SHAPING_CACHE_N; i++) {
        if (c->shaping[i].s) {
            canvas2d_shaped_free(c->shaping[i].s);  // releases the runs' CTFontRefs too
    ...
    canvas2d_text_cache_init(c);  // back to the empty state, stats included
}

// The slot a fresh shaped line lands in: the first empty slot, else the
// least-recently-used one.
static struct canvas2d_shaping_slot *__single shaping_lru_victim(struct canvas2d_text_cache *__single c) {
    struct canvas2d_shaping_slot *victim = &c->shaping[0];
    for (int i = 0; i < CANVAS2D_SHAPING_CACHE_N; i++) {
        if (!c->shaping[i].s) {
            return &c->shaping[i];
        }
...
struct canvas2d_shaped const *__single canvas2d_text_cache_shaping(struct canvas2d_text_cache *__single c,
    ...
    struct canvas2d_shaping_slot *__single hit = canvas2d_text_cache_shaping_slot(c, name, name_len,
    ...
    struct canvas2d_shaped *__single s = canvas2d_shape_text(name, name_len, size_px, rtl,
    ...
    canvas2d_shaped_apply_spacing(s, text, len, ls, ws);
    ...
    struct canvas2d_shaping_slot *victim = shaping_lru_victim(c);
    if (victim->s) {
        canvas2d_shaped_free(victim->s);
        free(victim->text);
        free(victim->fam);
        free(victim->lang);
    }
...
int canvas2d_text_cache_intern(struct canvas2d_text_cache *__single c,
    ...
    if (c->nfonts == CANVAS2D_FONT_INTERN_N) {
...
int canvas2d_text_cache_font(struct canvas2d_text_cache *__single c, void *__single font,
    ...
    char buf[256];
    int const n = canvas2d_run_font_name(font, buf, (int)sizeof buf);
    if (n <= 0) {
        return -1;
    }
    int const fid = canvas2d_text_cache_intern(c, buf, n, weight, italic);
    ...
    if (fid >= 0 && !c->font[fid].has_vm) {
        ...
        canvas2d_run_vmetrics(font, &a1, &d1);
        canvas2d_text_cache_set_vmetrics(c, fid, a1, d1);
    }
    return fid;
}
...
// CANVAS2D_GLYPH_CACHE_N < CANVAS2D_GLYPH_TABLE_N entries, so an unused slot always
// terminates the probe.
static struct canvas2d_glyph_slot *__single glyph_probe(struct canvas2d_text_cache *__single c,
                                             uint32_t key) {
    if (c->glyph_cap == 0) {
        return NULL;
    }
    ...
    for (uint32_t i = mix32(key) & mask;; i = (i + 1u) & mask) {
        struct canvas2d_glyph_slot *slot = &c->glyph[i];
...
static bool glyph_table_ensure(struct canvas2d_text_cache *__single c) {
    if (c->glyph_cap == 0) {
        struct canvas2d_glyph_slot *t = calloc(CANVAS2D_GLYPH_TABLE_N, sizeof *t);
        if (t) {
            c->glyph = t;
            c->glyph_cap = CANVAS2D_GLYPH_TABLE_N;
        }
    }
...
struct canvas2d_glyph_slot *__single canvas2d_text_cache_glyph(struct canvas2d_text_cache *__single c,
    ...
    struct canvas2d_glyph_slot *slot = glyph_probe(c, key);
    if (slot && slot->used) {
        c->glyph_hits++;
        return slot;
    }
    ...
    if (!slot || c->glyph_count >= CANVAS2D_GLYPH_CACHE_N) {
    ...
    enum canvas2d_glyph_verb vstack[VSTACK];
    canvas2d_vec2 pstack[PSTACK];
    int nv = 0, np = 0;
    float upem = 0.0f;
    canvas2d_glyph_curves(font, glyph, vstack, VSTACK, pstack, PSTACK, &nv, &np, &upem);
    ...
    if (blank) {  // remembered too -- "no outline" is itself a boundary answer
        ...
        enum canvas2d_glyph_verb *vheap = malloc((size_t)nv * sizeof *vheap);
        canvas2d_vec2 *pheap = malloc((size_t)(np > 0 ? np : 1) * sizeof *pheap);
        if (!vheap || !pheap) {
            free(vheap);
            free(pheap);
        ...
        int nv2 = 0, np2 = 0;
        canvas2d_glyph_curves(font, glyph, vheap, nv, pheap, np, &nv2, &np2, &upem);
        ...
        enum canvas2d_glyph_verb *vc = malloc((size_t)nv * sizeof *vc);
        canvas2d_vec2 *pc = malloc((size_t)(np > 0 ? np : 1) * sizeof *pc);
        if (!vc || !pc) {
            free(vc);
            free(pc);
            return NULL;
    ...
    // the curves (canvas2d_glyph_bounds reports px at the size `font` is built at),
...
void canvas2d_text_cache_put_glyph(struct canvas2d_text_cache *__single c, int fid,
    ...
    uint32_t key = ((uint32_t)fid << 16) | (uint32_t)glyph;
    struct canvas2d_glyph_slot *slot = glyph_probe(c, key);
    if (!slot || slot->used || c->glyph_count >= CANVAS2D_GLYPH_CACHE_N) {
...
// at CANVAS2D_CAPTURE_EM px to the em, fetched through the boundary once ever and
...
struct canvas2d_glyph_slot *__single canvas2d_text_cache_color(struct canvas2d_text_cache *__single c,
    ...
    // edge).
    void *__single big = canvas2d_font_resized(font, (float)CANVAS2D_CAPTURE_EM);
    if (!big) {
        return NULL;
    }
    float x0 = 0.0f, y0 = 0.0f, x1 = 0.0f, y1 = 0.0f;
    canvas2d_glyph_bounds(big, glyph, &x0, &y0, &x1, &y1);
    if (x1 <= x0 || y1 <= y0) {  // blank color glyph: remembered as no-capture,
        canvas2d_font_release(big);  // so it costs one boundary query ever
        slot->key = key;
        slot->used = true;
        slot->emitted = false;
        c->glyph_count += 1;
    ...
    int const w = CANVAS2D_CAPTURE_EM, h = CANVAS2D_CAPTURE_EM;
    int const len = w * h * 4;
    uint8_t *px = calloc((size_t)len, 1);  // transparent ground
    if (!px) {
        canvas2d_font_release(big);
        return NULL;  // OOM: the caller takes the per-draw boundary path
    }
    canvas2d_glyph_draw(big, glyph, -x0, -y0, px, w, h);
    canvas2d_font_release(big);
    slot->capture = px;
...
static void build_mips(struct canvas2d_glyph_slot *__single slot) {
    ...
    canvas2d_mip *m = calloc((size_t)n, sizeof *m);
    if (!m) {
        return;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        canvas2d_mip_halve(prev, pw, ph, px, lw, lh);
        m[i].px = px;
        m[i].len = llen;
        m[i].w = lw;
        m[i].h = lh;
...
canvas2d_mip canvas2d_glyph_mip(struct canvas2d_glyph_slot *__single slot, float footprint) {
    canvas2d_mip pick = { .px = NULL, .len = 0, .w = 0, .h = 0 };
...
float canvas2d_glyph_mip_pair(struct canvas2d_glyph_slot *__single slot, float footprint,
    ...
    *fine = (canvas2d_mip){ .px = NULL, .len = 0, .w = 0, .h = 0 };
    *coarse = *fine;
    if (!slot || slot->capture_w <= 0) {
        return 0.0f;
    }
    build_mips(slot);
    *fine = (canvas2d_mip){ .px = slot->capture, .len = slot->capture_size,
...
struct canvas2d_shaping_slot *__single canvas2d_text_cache_shaping_slot(struct canvas2d_text_cache *__single c,
    ...
    memcpy(&ws_bits, &ws, sizeof ws_bits);
    for (int i = 0; i < CANVAS2D_SHAPING_CACHE_N; i++) {
        struct canvas2d_shaping_slot *slot = &c->shaping[i];
...
void canvas2d_text_cache_put_shaping(struct canvas2d_text_cache *__single c,
    ...
    if (!copy || !fam || !langcopy) {
        ...
        free(fam);
        free(langcopy);
        canvas2d_shaped_free(s);  // can't key it: the usual best-effort degradation
        return;
    }
    ...
    struct canvas2d_shaping_slot *victim = canvas2d_text_cache_shaping_slot(c, name, name_len,
                                              size_px, rtl, ls, ws, weight, italic,
                                              kerning, rendering, variant_caps, stretch,
                                              lang, lang_len, text, len);
    if (!victim) {
...
static void canvas2d_glyph_outline(void *__single font, uint16_t glyph, float size_px,
    ...
    enum canvas2d_glyph_verb *vheap = NULL;
    canvas2d_vec2 *pheap = NULL;
...
// canvas2d_glyph_outline) fetches and walks without remembering.  A replay-built
...
static void glyph_outline_cached(struct canvas2d_text_cache *__single c, int fid,
    ...
    if (font) {
        canvas2d_glyph_outline(font, glyph, size_px, ox, oy, to_device, tol, out);
    }
}
...
float canvas2d_shaped_outline(struct canvas2d_text_cache *__single cache,
    ...
    float pen = ox;
    for (int r = 0; r < s->nruns; r++) {
        struct canvas2d_glyph_run const run = s->run[r];  // visual-order glyphs, so advancing the pen
        ...
        int fid = run.name_id >= 0 ? run.name_id
                                   : canvas2d_text_cache_font(cache, run.font, s->weight, s->italic);
...
// capture-px box by size_px/CANVAS2D_CAPTURE_EM.  When the cache can't serve, a
// run that still has its font handle measures through a live
// canvas2d_glyph_bounds, and a handle-less run's glyph adds no ink.
void canvas2d_measure_shaped(struct canvas2d_text_cache *__single cache,
                         ...
                         canvas2d_shaped_metrics *__single m) {
    memset(m, 0, sizeof *m);
    ...
    for (int r = 0; r < s->nruns; r++) {
        ...
        for (int i = 0; i < run.count; i++) {
            float x0 = 0.0f, y0 = 0.0f, x1 = 0.0f, y1 = 0.0f;
            struct canvas2d_glyph_slot *slot = run.is_color
                ? canvas2d_text_cache_color(cache, fid, run.font, run.glyph[i])
                : canvas2d_text_cache_glyph(cache, fid, run.font, run.glyph[i],
                                        size_px);
            if (slot) {
                float k = 0.0f;  // a blank glyph's box stays all-zero
                if (run.is_color && slot->capture_w > 0) {
                    k = size_px / (float)CANVAS2D_CAPTURE_EM;
                ...
                canvas2d_glyph_bounds(run.font, run.glyph[i], &x0, &y0, &x1, &y1);
            }
            if (x1 > x0 && y1 > y0) {
