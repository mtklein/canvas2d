...
static canvas2d_filter scale_offset(float k, float c) {
    return (canvas2d_filter){
...
canvas2d_filter canvas2d_filter_brightness(float amount) {
...
canvas2d_filter canvas2d_filter_contrast(float amount) {
    return scale_offset(amount, 0.5f - 0.5f * amount);
}

// invert(k) lerps each channel toward its complement: c' = c*(1 - 2k) + k.
canvas2d_filter canvas2d_filter_invert(float amount) {
    return scale_offset(1.0f - 2.0f * amount, amount);
}

canvas2d_filter canvas2d_filter_opacity(float amount) {
    canvas2d_filter f = scale_offset(amount, 0.0f);
    f.ka = amount;
    return f;
}
...
static canvas2d_filter saturate_matrix(float s) {
    return (canvas2d_filter){
        .m = {
...
canvas2d_filter canvas2d_filter_saturate(float amount) {
...
canvas2d_filter canvas2d_filter_grayscale(float amount) {
    return saturate_matrix(1.0f - amount);
}

// sepia(g) interpolates identity (t = 1) toward the spec's sepia matrix (t = 0).
canvas2d_filter canvas2d_filter_sepia(float amount) {
...
canvas2d_filter canvas2d_filter_hue_rotate(float radians) {
...
canvas2d_filter canvas2d_filter_blur(int radius) {
    canvas2d_filter f = scale_offset(1.0f, 0.0f);
    f.blur = radius;
    return f;
}
...
// property as blur().
canvas2d_filter canvas2d_filter_drop_shadow(int dx, int kx, int dy, int ky, int radius,
...
// channel is a whole plane (canvas2d_planar.h).
static canvas2d_px8 filter_block(canvas2d_px8 p, canvas2d_filter const *__single fn) {
    canvas2d_px8 q;
...
void canvas2d_filter_apply(canvas2d_filter const *__counted_by(count) list, int count,
                       canvas2d_premul *__counted_by(n) px, int n) {
    ...
    for (int f = 0; f < count; f++) {
        canvas2d_filter const fn = list[f];
        int i = 0;
        for (; i + 8 <= n; i += 8) {
            canvas2d_px8_store(px + i, filter_block(canvas2d_px8_load(px + i), &fn));
        }
        if (i < n) {
            int const k = n - i;
            canvas2d_px8_store_k(px + i, k,
                             filter_block(canvas2d_px8_load_k(px + i, k), &fn));
        }
    }
}
