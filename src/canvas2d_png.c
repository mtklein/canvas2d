...
// the memcpy idiom -- one bounds check per 16-byte block (the struct canvas2d_cover
...
canvas2d_png_encode(uint16_t const *__counted_by(width * height * 4) pixels,
    ...
    *outlen = 0;
    ...
    // works in, with 1/256 headroom so canvas2d_zlib_bound (~n*1.001) fits too: a
    ...
    int const zcap = canvas2d_zlib_bound(rawlen);
    uint8_t *__counted_by_or_null(zcap) z = malloc((size_t)zcap);
    if (!z) {
        free(raw);
        return NULL;
    }
    int const zn = canvas2d_zlib_deflate(z, zcap, raw, rawlen);
    free(raw);
...
bool canvas2d_png_write(char const *__null_terminated path,
    ...
    int total = 0;
    uint8_t *out = canvas2d_png_encode(pixels, width, height, &total);
    if (!out) {
        return false;
    }
    bool ok = false;
