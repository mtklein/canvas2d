...
#define CANVAS2D_STOPS_MAX 16

enum canvas2d_grad_kind { CANVAS2D_GRAD_LINEAR, CANVAS2D_GRAD_RADIAL, CANVAS2D_GRAD_CONIC };

typedef struct {
    float offset;  // in [0,1]
    canvas2d_unpremul color;
} canvas2d_stop;
...
struct canvas2d_gradient {
    enum canvas2d_grad_kind kind;
    canvas2d_vec2 p0, p1;
    float r0, r1;
    ...
    // The DEFAULT (CANVAS2D_CS_SRGB + CANVAS2D_ALPHA_UNPREMUL, the zero values for
    ...
    // keeps the colour kernels (canvas2d_gradient_color_at/_row) dependent only on
    // the gradient, not the canvas.
    enum canvas2d_color_space interp;       // the gradient lerp SPACE
    enum canvas2d_alpha_type  interp_alpha; // premultiply colour coords before lerp?
    enum canvas2d_color_space space;        // the canvas working space (sRGB or linear)
};
...
// solve.  Same math as canvas2d_gradient_param, only the geometry coords differ.
...
// Equivalent to calling canvas2d_gradient_param per pixel, 8 wide along the row.
...
// of the fill path; t is canvas2d_gradient_param_row's output).  out[i] is
// bit-identical to canvas2d_gradient_color_at(gr, t[i]) -- the exact piecewise-
