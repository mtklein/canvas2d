...
// copies each run into checked-owned struct canvas2d_glyph_run arrays for the checked core.
...
// Map the nine fontStretch keywords (canvas2d_font_stretch 0..8, normal == 4) onto
...
// raster-only effect.
#define CANVAS2D_SYNTH_ITALIC_SHEAR 0.25

// fontStretch NORMAL: the centre of the width axis (canvas2d_font_stretch 4), the
...
#define CANVAS2D_STRETCH_NORMAL 4
...
static CTFontRef font_descriptor(CFStringRef family, CGFloat size,
    ...
    CFNumberRef dnum = stretch != CANVAS2D_STRETCH_NORMAL
        ? CFNumberCreate(NULL, kCFNumberCGFloatType, &wd) : NULL;
    CTFontRef out = NULL;
    if (wnum && snum && (dnum || stretch == CANVAS2D_STRETCH_NORMAL)) {
...
static CTFontRef font_with_traits(CFStringRef family, CGFloat size,
    ...
    if (out && italic &&
        ...
        CGAffineTransform m = { 1.0, 0.0, CANVAS2D_SYNTH_ITALIC_SHEAR, 1.0, 0.0, 0.0 };
...
int canvas2d_run_font_name(void const *font, char *buf, int cap) {
    if (!font || cap <= 0) {
        return -1;
    }
...
void canvas2d_shaped_free(struct canvas2d_shaped *s) {
    if (!s) {
        return;
    }
    if (s->run) {
...
static bool copy_run(CTRunRef run, struct canvas2d_glyph_run *dst) {
...
// run in the checked core now (canvas2d_glyph_outline in canvas2d_text.c).
struct curves {
    enum canvas2d_glyph_verb *verb;
    canvas2d_vec2 *pt;
    int vcap, pcap;
...
static void put(struct curves *c, enum canvas2d_glyph_verb v, CGPoint const *p, int n) {
    if (c->nv < c->vcap) {
        c->verb[c->nv] = v;
    }
    c->nv++;
    for (int i = 0; i < n; i++) {
        if (c->np < c->pcap) {
            c->pt[c->np] = (canvas2d_vec2){ (float)(p[i].x * c->to_units),
...
static void emit(void *info, CGPathElement const *e) {
    ...
    switch (e->type) {
        case kCGPathElementMoveToPoint:
            put(c, CANVAS2D_GLYPH_MOVE, e->points, 1);
            break;
        case kCGPathElementAddLineToPoint:
            put(c, CANVAS2D_GLYPH_LINE, e->points, 1);
            break;
        case kCGPathElementAddQuadCurveToPoint:
            put(c, CANVAS2D_GLYPH_QUAD, e->points, 2);
            break;
        case kCGPathElementAddCurveToPoint:
            put(c, CANVAS2D_GLYPH_CUBIC, e->points, 3);
            break;
        case kCGPathElementCloseSubpath:
            put(c, CANVAS2D_GLYPH_CLOSE, e->points, 0);
            break;
    }
}

void canvas2d_glyph_curves(void *font, uint16_t glyph,
                       enum canvas2d_glyph_verb *verb, int vcap,
                       canvas2d_vec2 *pt, int pcap,
...
void canvas2d_glyph_draw(void *font, uint16_t glyph, float x, float y,
...
void *canvas2d_font_resized(void *font, float size_px) {
...
void canvas2d_font_release(void *font) {
    if (font) {
        CFRelease(font);
    }
}

void canvas2d_run_vmetrics(void const *font, float *asc1, float *desc1) {
    *asc1 = 0.0f;
    *desc1 = 0.0f;
    if (!font) {
        return;
...
void canvas2d_glyph_bounds(void *font, uint16_t glyph, float *x0, float *y0,
                       float *x1, float *y1) {
...
// public canvas2d_font_kerning / canvas2d_text_rendering enums (passed as ints to
...
// canvas2d_font_variant_caps values (kept off the boundary ABI like the others):
...
struct canvas2d_shaped *canvas2d_shape_text(char const *name, int name_len, float size_px, bool rtl,
    ...
    struct canvas2d_shaped *out = NULL;
    if (font && str && ps) {
...
struct canvas2d_font *canvas2d_font(char const *name, int name_len, float size_px,
                            int weight, bool italic, int stretch) {
    CFStringRef cfname = str_from_bytes(name, name_len);
    if (!cfname) {
        return NULL;
    ...
    struct canvas2d_font *f = calloc(1, sizeof *f);
    if (!f) {
        CFRelease(font);
        return NULL;
    }
...
void canvas2d_font_free(struct canvas2d_font *f) {
    if (!f) {
        return;
    }
    CFRelease(f->font);
...
void canvas2d_font_vmetrics(struct canvas2d_font *f, float *ascent, float *descent) {
