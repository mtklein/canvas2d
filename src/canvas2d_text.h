...
// (canvas2d_text_ct.c, built without -fbounds-safety to bind the un-annotated CoreText
...
// (canvas2d_text.c) then does layout and hit-testing fully bounds-checked.
//
...
struct canvas2d_shaped {
    struct canvas2d_glyph_run *__counted_by(nruns) run;
    int nruns;
...
// `weight` (CSS 100..900), `italic`, and `stretch` (canvas2d_font_stretch 0..8 as
...
// the canvas2d_font_kerning / canvas2d_text_rendering / canvas2d_font_variant_caps
...
void canvas2d_shaped_free(struct canvas2d_shaped *__single s);

// Checked-core consumers.
float canvas2d_shaped_width(struct canvas2d_shaped const *__single s);                // sum of advances
...
void canvas2d_shaped_apply_spacing(struct canvas2d_shaped *__single s,
                               char const *__counted_by(text_len) text, int text_len,
                               float ls, float ws);
int canvas2d_shaped_index_at_x(struct canvas2d_shaped const *__single s, float x);    // hit-test -> UTF-16 index, or -1
...
float canvas2d_shaped_x_at_index(struct canvas2d_shaped const *__single s, int index);
...
int canvas2d_shaped_selection(struct canvas2d_shaped const *__single s, int lo, int hi,
                          canvas2d_xspan *__counted_by(max) out, int max);
...
int canvas2d_run_font_name(void const *__single font, char *__counted_by(cap) buf, int cap);
...
enum canvas2d_glyph_verb : uint8_t {
    CANVAS2D_GLYPH_MOVE,   // 1 point
    CANVAS2D_GLYPH_LINE,   // 1 point
    CANVAS2D_GLYPH_QUAD,   // 2 points: control, end
    CANVAS2D_GLYPH_CUBIC,  // 3 points: control 1, control 2, end
    CANVAS2D_GLYPH_CLOSE,  // no points
};

// Fetch one glyph's canonical curves.  The checked caller owns the buffers and the
// boundary fills within their caps (the canvas2d_glyph_draw hand-off, twice over); the
...
void canvas2d_glyph_curves(void *__single font, uint16_t glyph,
                       enum canvas2d_glyph_verb *__counted_by(vcap) verb, int vcap,
                       canvas2d_vec2 *__counted_by(pcap) pt, int pcap,
...
//     struct canvas2d_shaped.  The family is IN the key (the REQUESTED name, not the
...
//     advances (canvas2d_shaped_apply_spacing) before it is interned, so two
...
//     sites borrow.  Fixed CANVAS2D_SHAPING_CACHE_N slots, evicted
...
//     canonical
//     form: font-unit verbs/points + units_per_em from canvas2d_glyph_curves for
//     an outline glyph, or the fixed-size premultiplied capture from one
//     canvas2d_glyph_draw for a color (emoji) glyph.  Canonical bytes are size-
...
//     Open-addressed, fixed CANVAS2D_GLYPH_TABLE_N slots; inserts simply stop at
//     CANVAS2D_GLYPH_CACHE_N entries (no eviction: canonical bytes never go
...
enum {
    CANVAS2D_SHAPING_CACHE_N = 64,    // shaped-line slots (LRU eviction)
    CANVAS2D_GLYPH_CACHE_N = 1024,  // max cached glyphs (inserts stop, no eviction)
    CANVAS2D_GLYPH_TABLE_N = 2048,  // open-addressing slots: 2x entries, power of two
    CANVAS2D_FONT_INTERN_N = 16,    // distinct font names (primary + fallbacks)
    CANVAS2D_CAPTURE_EM = 160,      // color-glyph capture size: the em in capture px
...
struct canvas2d_shaping_slot {
    ...
    int kerning;         // canvas2d_font_kerning, the key's kerning part
    int rendering;       // canvas2d_text_rendering, the key's rendering part
    int variant_caps;    // canvas2d_font_variant_caps, the key's small-caps part
    int stretch;         // canvas2d_font_stretch, the key's width part
    ...
    struct canvas2d_shaped *__single s;  // owned; freed on eviction/clear
    bool emitted;  // already serialized into the active recording (canvas2d_record.c)
};
...
// of the glyph at CANVAS2D_CAPTURE_EM px to the em, rasterized by the boundary
...
struct canvas2d_glyph_slot {
    enum canvas2d_glyph_verb *__counted_by(nverbs) verb;  // owned canonical curves
    canvas2d_vec2 *__counted_by(npts) pt;
    int nverbs, npts;
    ...
    float ink_x0, ink_y0, ink_x1, ink_y1;  // tight ink box, y up, baseline-
                   ...
                   // glyph (scale by size_px/CANVAS2D_CAPTURE_EM).  Size-independent
    ...
    // canvas2d_glyph_draw + canvas2d_glyph_bounds place a live render.
    uint8_t *__counted_by(capture_size) capture;  // owned; the canonical bytes
    int capture_size;       // capture_w * capture_h * 4
    int capture_w, capture_h;  // capture dims (CANVAS2D_CAPTURE_EM square when fetched live)
    canvas2d_mip *__counted_by(nmips) mip;  // derived pyramid, largest first, down
    int nmips;                          // to 1x1; built lazily by canvas2d_glyph_mip
...
struct canvas2d_text_cache {
    struct canvas2d_shaping_slot shaping[CANVAS2D_SHAPING_CACHE_N];
    uint64_t tick;  // monotone use counter feeding the shape slots' last_use
    struct canvas2d_glyph_slot *__counted_by(glyph_cap) glyph;  // lazily built on first miss
    int glyph_cap;                                   // 0 until that build succeeds
    int glyph_count;
    struct canvas2d_font_name font[CANVAS2D_FONT_INTERN_N];  // interned names; index = font id
    int nfonts;
...
void canvas2d_text_cache_init(struct canvas2d_text_cache *__single c);   // an empty cache
void canvas2d_text_cache_reset(struct canvas2d_text_cache *__single c);  // free entries -> empty;
...
// advances (canvas2d_shaped_apply_spacing) on a miss before caching.  The result is
...
// somewhere to live).
struct canvas2d_shaped const *__single canvas2d_text_cache_shaping(struct canvas2d_text_cache *__single c,
...
// records the font's vmetrics (canvas2d_run_vmetrics).
int canvas2d_text_cache_font(struct canvas2d_text_cache *__single c, void *__single font,
...
// canvas2d_text_cache_font.
int canvas2d_text_cache_intern(struct canvas2d_text_cache *__single c,
                           char const *__counted_by(len) name, int len,
                           int weight, bool italic);

// The per-name vertical metrics record (asc1/desc1 in struct canvas2d_font_name): set
...
void canvas2d_text_cache_set_vmetrics(struct canvas2d_text_cache *__single c, int fid,
                                  float asc1, float desc1);
bool canvas2d_text_cache_get_vmetrics(struct canvas2d_text_cache *__single c, int fid,
...
struct canvas2d_glyph_slot *__single canvas2d_text_cache_glyph(struct canvas2d_text_cache *__single c,
...
void canvas2d_text_cache_put_glyph(struct canvas2d_text_cache *__single c, int fid,
        uint16_t glyph, enum canvas2d_glyph_verb *__counted_by(nverbs) verb, int nverbs,
        canvas2d_vec2 *__counted_by(npts) pt, int npts, float upem,
...
// ONCE -- a sized handle at CANVAS2D_CAPTURE_EM px (canvas2d_font_resized), the ink box
// (canvas2d_glyph_bounds, in capture px), and one canvas2d_glyph_draw into the square
// CANVAS2D_CAPTURE_EM buffer -- and inserts; every later draw at every size and
...
// advance).
struct canvas2d_glyph_slot *__single canvas2d_text_cache_color(struct canvas2d_text_cache *__single c,
...
void canvas2d_text_cache_put_capture(struct canvas2d_text_cache *__single c, int fid,
...
// otherwise).  Exported for the pyramid tests; canvas2d_glyph_mip drives it.
void canvas2d_mip_halve(uint8_t const *__counted_by(sw * sh * 4) src, int sw, int sh,
...
// no capture.
canvas2d_mip canvas2d_glyph_mip(struct canvas2d_glyph_slot *__single slot, float footprint);

// The trilinear flavour of canvas2d_glyph_mip: borrow the pyramid level PAIR
...
// no capture.
float canvas2d_glyph_mip_pair(struct canvas2d_glyph_slot *__single slot, float footprint,
                          canvas2d_mip *__single fine, canvas2d_mip *__single coarse);
...
// the same key.
void canvas2d_text_cache_put_shaping(struct canvas2d_text_cache *__single c,
        char const *__counted_by(name_len) name, int name_len, float size_px,
        bool rtl, float ls, float ws, int weight, bool italic,
        int kerning, int rendering, int variant_caps, int stretch,
        char const *__counted_by(lang_len) lang, int lang_len,
        char const *__counted_by(len) text, int len,
        struct canvas2d_shaped *__single s);
...
struct canvas2d_shaping_slot *__single canvas2d_text_cache_shaping_slot(struct canvas2d_text_cache *__single c,
        char const *__counted_by(name_len) name, int name_len, float size_px,
        bool rtl, float ls, float ws, int weight, bool italic,
        int kerning, int rendering, int variant_caps, int stretch,
        char const *__counted_by(lang_len) lang, int lang_len,
...
// Callback for the color (emoji) glyphs canvas2d_shaped_outline meets: they have no
...
typedef void (*canvas2d_color_glyph_fn)(void *__single ctx, int fid,
...
void canvas2d_glyph_draw(void *__single font, uint16_t glyph, float x, float y,
                     uint8_t *__counted_by(w * h * 4) px, int w, int h);

// A sized copy of an opaque run-font handle: the same face at `size_px` -- the
// capture-size handle canvas2d_text_cache_color rasterizes with.  Retained; release
// with canvas2d_font_release.  NULL on failure.  Boundary helper: an opaque handle
// in, an opaque handle out, no bounds to carry.
void *__single canvas2d_font_resized(void *__single font, float size_px);
void canvas2d_font_release(void *__single font);
...
// serves every size.
void canvas2d_run_vmetrics(void const *__single font, float *__single asc1,
...
// sizes and places the buffer canvas2d_glyph_draw fills.
void canvas2d_glyph_bounds(void *__single font, uint16_t glyph,
...
// it through the same trait-applying descriptor as canvas2d_shape_text (a real width
// face resolves a distinct name, so its vmetrics record stays separate).  NULL
// on failure.  Like canvas2d_shape_text, the name crosses the boundary counted --
// (bytes, len), no NUL contract.
struct canvas2d_font;
struct canvas2d_font *__single canvas2d_font(char const *__counted_by(name_len) name,
                                     int name_len, float size_px,
                                     int weight, bool italic, int stretch);
void canvas2d_font_free(struct canvas2d_font *__single f);
...
void canvas2d_font_vmetrics(struct canvas2d_font *__single f, float *__single ascent,
...
void canvas2d_measure_shaped(struct canvas2d_text_cache *__single cache,
                         struct canvas2d_shaped const *__single s, float size_px,
                         float ascent_px, float descent_px,
                         canvas2d_shaped_metrics *__single out);
