...
struct rec_image {
    ...
    int len;
    int w, h;
    enum canvas2d_color_type ct;  // the block's colour type, written by name
...
// The text format's enum spellings (canvas2d_record.h): written here, parsed by
// canvas2d_replay.c, one table for both directions.
char const *const canvas2d_composite_name[CANVAS2D_OP_LUMINOSITY + 1] = {
...
char const *const canvas2d_repeat_name[CANVAS2D_NO_REPEAT + 1] = {
...
char const *const canvas2d_color_space_name[CANVAS2D_CS_OKLAB + 1] = {
    "srgb", "linear", "oklab",
};
...
char const *const canvas2d_alpha_type_name[CANVAS2D_ALPHA_PREMUL + 1] = {
    "unpremul", "premul",
};
...
struct canvas2d_recorder {
    ...
    enum canvas2d_color_space pending_space;
    bool wrote_ws;  // the `working_space` line has been flushed for this file
    struct rec_image img[CANVAS2D_REC_IMAGES_MAX];  // [0, nimg) are this file's image blocks
    int nimg;
    struct rec_path path[CANVAS2D_REC_PATHS_MAX];   // [0, npath) are its path blocks
    int npath;
};
...
struct canvas2d_recorder *__single canvas2d_recorder_begin(char const *__null_terminated path) {
    ...
    struct canvas2d_recorder *__single r = calloc(1, sizeof *r);
    if (!r) {
        (void)fclose(f);
        return NULL;
    }
...
static void canvas2d_rec_flush_ws(struct canvas2d_recorder *__single r) {
    if (r->wrote_ws) {
        return;
    }
    r->wrote_ws = true;  // latch first: a malformed space still settles the head
    unsigned const i = (unsigned)r->pending_space;
    if (i >= sizeof canvas2d_color_space_name / sizeof canvas2d_color_space_name[0]) {
        return;
    }
    fputs("working_space ", r->f);
    fputs(canvas2d_color_space_name[i], r->f);
    fputc('\n', r->f);
}
...
// (walk_curves in canvas2d_text.c): a bad byte or a short point array stops the
// walk instead of being trusted.
static char const *__null_terminated verb_token(enum canvas2d_glyph_verb v,
                                                int *__single npts) {
    switch (v) {
        case CANVAS2D_GLYPH_MOVE:  *npts = 1; return "m";
        case CANVAS2D_GLYPH_LINE:  *npts = 1; return "l";
        case CANVAS2D_GLYPH_QUAD:  *npts = 2; return "q";
        case CANVAS2D_GLYPH_CUBIC: *npts = 3; return "c";
        case CANVAS2D_GLYPH_CLOSE: *npts = 0; return "z";
    }
    *npts = 0;
    return NULL;
}
...
static int run_fid(struct canvas2d_text_cache *__single c, struct canvas2d_glyph_run const *__single run,
                   int weight, bool italic) {
    if (run->name_id >= 0) {
        return run->name_id;
    }
    return canvas2d_text_cache_font(c, run->font, weight, italic);
}
...
enum { CANVAS2D_REC_BITS_PER_LINE = 12288 };
...
// The shaped line itself: everything struct canvas2d_shaped carries, so replay
...
static void canvas2d_rec_shaping_line(struct canvas2d_recorder *__single r,
                                  struct canvas2d_text_cache *__single c,
                                  struct canvas2d_shaped const *__single s,
    ...
    for (int ri = 0; ri < s->nruns; ri++) {
        struct canvas2d_glyph_run const *__single run = &s->run[ri];
...
void canvas2d_rec_text_blocks(struct canvas2d_recorder *__single r, struct canvas2d_text_cache *__single c,
    ...
    canvas2d_rec_flush_ws(r);
    struct canvas2d_shaping_slot *__single slot = canvas2d_text_cache_shaping_slot(c, family, fam_len,
    ...
    struct canvas2d_shaped const *__single s = slot->s;
    if (slot->emitted) {
    ...
    for (int ri = 0; ri < s->nruns; ri++) {
        ...
        for (int i = 0; i < run->count; i++) {
            struct canvas2d_glyph_slot *__single g =
                canvas2d_text_cache_glyph(c, fid, run->font, run->glyph[i], size_px);
            if (!g || g->emitted) {
                continue;
            }
    ...
    // deflated (canvas2d_zlib) and the COMPRESSED stream chunked into base64
    ...
    for (int ri = 0; ri < s->nruns; ri++) {
        ...
        for (int i = 0; i < run->count; i++) {
            struct canvas2d_glyph_slot *__single g =
                canvas2d_text_cache_color(c, fid, run->font, run->glyph[i]);
            if (!g || g->emitted || g->capture_w <= 0) {
                continue;
            }
            int const zcap = canvas2d_zlib_bound(g->capture_size);
            uint8_t *__counted_by_or_null(zcap) z = malloc((size_t)zcap);
            if (!z) {
                continue;  // OOM: skip the block, leave the glyph un-emitted
            }
            int const zn = canvas2d_zlib_deflate(z, zcap, g->capture, g->capture_size);
            if (zn < 0) {
                free(z);
                continue;  // deflate's own scratch allocation failed: ditto
            }
            int const nlines =
                (zn + CANVAS2D_REC_BITS_PER_LINE - 1) / CANVAS2D_REC_BITS_PER_LINE;
            ...
            for (int off = 0; off < zn; off += CANVAS2D_REC_BITS_PER_LINE) {
                int const rem = zn - off;
                put_bits_line(r->f, z + off,
                              rem < CANVAS2D_REC_BITS_PER_LINE ? rem
                                                           : CANVAS2D_REC_BITS_PER_LINE);
            }
            free(z);
            g->emitted = true;
        }
    ...
    canvas2d_rec_shaping_line(r, c, s, family, fam_len, size_px, rtl, ls, ws,
...
int canvas2d_rec_image(struct canvas2d_recorder *__single r,
    ...
    int const bpp = ct == CANVAS2D_COLOR_F16 ? 8 : 4;
    if (w < 1 || h < 1 || (int64_t)w * h * bpp != (int64_t)len ||
        len > CANVAS2D_REC_IMAGE_BYTES_MAX) {
    ...
    if (r->nimg >= CANVAS2D_REC_IMAGES_MAX) {
    ...
    int const zcap = canvas2d_zlib_bound(len);
    uint8_t *__counted_by_or_null(zcap) z = malloc((size_t)zcap);
    if (!z) {
        free(copy);
        return -1;
    }
    int const zn = canvas2d_zlib_deflate(z, zcap, px, len);
    if (zn < 0) {
        free(z);
        free(copy);
    ...
    int const id = r->nimg;
    int const nlines = (zn + CANVAS2D_REC_BITS_PER_LINE - 1) / CANVAS2D_REC_BITS_PER_LINE;
    ...
    if (csi >= sizeof canvas2d_color_space_name / sizeof canvas2d_color_space_name[0]) {
        free(z);
        free(copy);
        return -1;
    }
    canvas2d_rec_flush_ws(r);  // the working_space line leads the file, ahead of any block
    fprintf(r->f, "image %d %s %s %d %d %d %d", id,
            ct == CANVAS2D_COLOR_F16 ? "f16" : "unorm8",
            at == CANVAS2D_ALPHA_PREMUL ? "premul" : "unpremul",
            w, h, zn, nlines);
    fputc(' ', r->f);
    fputs(canvas2d_color_space_name[csi], r->f);
    fputc('\n', r->f);
    for (int off = 0; off < zn; off += CANVAS2D_REC_BITS_PER_LINE) {
        int const rem = zn - off;
        put_bits_line(r->f, z + off,
                      rem < CANVAS2D_REC_BITS_PER_LINE ? rem : CANVAS2D_REC_BITS_PER_LINE);
    }
    free(z);
    r->img[id].len = len;
    r->img[id].px = copy;
...
int canvas2d_rec_path(struct canvas2d_recorder *__single r, struct canvas2d_path2d const *__single p) {
    if (!r || r->suspend != 0) {
        return -1;
    }
    int const n = p->ncmds;
    ...
    if (r->npath >= CANVAS2D_REC_PATHS_MAX) {
        return -1;  // id space exhausted: skip the block (and its op)
    }
...
void canvas2d_rec_path_rule(struct canvas2d_recorder *__single r,
    ...
    fputs(name, r->f);
    fprintf(r->f, " %d ", id);
    fputs(rule == CANVAS2D_EVENODD ? "evenodd" : "nonzero", r->f);
    fputc('\n', r->f);
}
...
void canvas2d_rec_smoothing_quality(struct canvas2d_recorder *__single r,
    ...
    switch (quality) {
        case CANVAS2D_SMOOTHING_LOW:    name = "low";    break;
        case CANVAS2D_SMOOTHING_MEDIUM: name = "medium"; break;
        case CANVAS2D_SMOOTHING_HIGH:   name = "high";   break;
    }
...
void canvas2d_rec_line_join(struct canvas2d_recorder *__single r, enum canvas2d_line_join join) {
    ...
    if (join == CANVAS2D_JOIN_ROUND) {
        name = "round";
    } else if (join == CANVAS2D_JOIN_BEVEL) {
        name = "bevel";
    }
...
void canvas2d_rec_line_cap(struct canvas2d_recorder *__single r, enum canvas2d_line_cap cap) {
    ...
    if (cap == CANVAS2D_CAP_ROUND) {
        name = "round";
    } else if (cap == CANVAS2D_CAP_SQUARE) {
        name = "square";
    }
...
void canvas2d_rec_text_align(struct canvas2d_recorder *__single r, enum canvas2d_text_align align) {
    ...
    switch (align) {
        case CANVAS2D_ALIGN_START:  name = "start";  break;
        case CANVAS2D_ALIGN_END:    name = "end";    break;
        case CANVAS2D_ALIGN_LEFT:   name = "left";   break;
        case CANVAS2D_ALIGN_RIGHT:  name = "right";  break;
        case CANVAS2D_ALIGN_CENTER: name = "center"; break;
    }
...
void canvas2d_rec_text_baseline(struct canvas2d_recorder *__single r,
    ...
    switch (baseline) {
        case CANVAS2D_BASELINE_ALPHABETIC:  name = "alphabetic";  break;
        case CANVAS2D_BASELINE_TOP:         name = "top";         break;
        case CANVAS2D_BASELINE_HANGING:     name = "hanging";     break;
        case CANVAS2D_BASELINE_MIDDLE:      name = "middle";      break;
        case CANVAS2D_BASELINE_IDEOGRAPHIC: name = "ideographic"; break;
        case CANVAS2D_BASELINE_BOTTOM:      name = "bottom";      break;
    }
...
void canvas2d_rec_direction(struct canvas2d_recorder *__single r, enum canvas2d_direction dir) {
    ...
    fputs(dir == CANVAS2D_DIRECTION_RTL ? "rtl" : "ltr", r->f);
    fputc('\n', r->f);
}
...
void canvas2d_rec_font_style(struct canvas2d_recorder *__single r, enum canvas2d_font_style style) {
    ...
    fputs(style == CANVAS2D_FONT_STYLE_ITALIC ? "italic" : "normal", r->f);
    fputc('\n', r->f);
}
...
void canvas2d_rec_font_kerning(struct canvas2d_recorder *__single r, enum canvas2d_font_kerning kerning) {
    ...
    switch (kerning) {
        case CANVAS2D_FONT_KERNING_AUTO:   tok = "auto";   break;
        case CANVAS2D_FONT_KERNING_NORMAL: tok = "normal"; break;
        case CANVAS2D_FONT_KERNING_NONE:   tok = "none";   break;
    }
...
void canvas2d_rec_text_rendering(struct canvas2d_recorder *__single r, enum canvas2d_text_rendering rendering) {
    ...
    char const *__null_terminated tok = "auto";
    switch (rendering) {
        case CANVAS2D_TEXT_RENDERING_AUTO:                tok = "auto";               break;
        case CANVAS2D_TEXT_RENDERING_OPTIMIZE_SPEED:      tok = "optimizeSpeed";      break;
        case CANVAS2D_TEXT_RENDERING_OPTIMIZE_LEGIBILITY: tok = "optimizeLegibility"; break;
        case CANVAS2D_TEXT_RENDERING_GEOMETRIC_PRECISION: tok = "geometricPrecision"; break;
    }
...
void canvas2d_rec_font_variant_caps(struct canvas2d_recorder *__single r,
    ...
    switch (variant_caps) {
        case CANVAS2D_FONT_VARIANT_CAPS_NORMAL:         tok = "normal";          break;
        case CANVAS2D_FONT_VARIANT_CAPS_SMALL_CAPS:     tok = "small-caps";      break;
        case CANVAS2D_FONT_VARIANT_CAPS_ALL_SMALL_CAPS: tok = "all-small-caps";  break;
    }
...
void canvas2d_rec_font_stretch(struct canvas2d_recorder *__single r, enum canvas2d_font_stretch stretch) {
    ...
    char const *__null_terminated tok = "normal";
    switch (stretch) {
        case CANVAS2D_FONT_STRETCH_ULTRA_CONDENSED: tok = "ultra-condensed"; break;
        case CANVAS2D_FONT_STRETCH_EXTRA_CONDENSED: tok = "extra-condensed"; break;
        case CANVAS2D_FONT_STRETCH_CONDENSED:       tok = "condensed";       break;
        case CANVAS2D_FONT_STRETCH_SEMI_CONDENSED:  tok = "semi-condensed";  break;
        case CANVAS2D_FONT_STRETCH_NORMAL:          tok = "normal";          break;
        case CANVAS2D_FONT_STRETCH_SEMI_EXPANDED:   tok = "semi-expanded";   break;
        case CANVAS2D_FONT_STRETCH_EXPANDED:        tok = "expanded";        break;
        case CANVAS2D_FONT_STRETCH_EXTRA_EXPANDED:  tok = "extra-expanded";  break;
        case CANVAS2D_FONT_STRETCH_ULTRA_EXPANDED:  tok = "ultra-expanded";  break;
    }
...
void canvas2d_rec_composite(struct canvas2d_recorder *__single r, enum canvas2d_composite_op op) {
    ...
    if (i >= sizeof canvas2d_composite_name / sizeof canvas2d_composite_name[0]) {
    ...
    fputs(canvas2d_composite_name[i], r->f);
    fputc('\n', r->f);
}
...
void canvas2d_rec_working_space(struct canvas2d_recorder *__single r,
    ...
    // is flushed lazily (canvas2d_rec_flush_ws) at the first other op, so a leading
...
void canvas2d_rec_gradient(struct canvas2d_recorder *__single r,
    ...
    if (si >= sizeof canvas2d_color_space_name / sizeof canvas2d_color_space_name[0] ||
        ai >= sizeof canvas2d_alpha_type_name / sizeof canvas2d_alpha_type_name[0]) {
        return;
    }
    fputs(name, r->f);
    fputc(' ', r->f);
    fputs(canvas2d_color_space_name[si], r->f);
    fputc(' ', r->f);
    fputs(canvas2d_alpha_type_name[ai], r->f);
    put_floats(r->f, v, n);
    fputc('\n', r->f);
}
